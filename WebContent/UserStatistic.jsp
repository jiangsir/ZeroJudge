				...
				<div class="panel panel-default">
					...
					<div class="panel-body">
						...
						<c:if
							...
							<div>
								題目分級統計：<br />
								<c:forEach var="levelcount" items="${allProblemlevelCounts }"
									varStatus="status">
									<c:if test="${levelcount.key==0}">未分級：</c:if>
									<c:if test="${levelcount.key>0 && levelcount.key<=5}"> APCS ${levelcount.key}級：</c:if>
									<c:if test="${levelcount.key>5}"> APCS 5級以上：</c:if>
									
									 (${userProblemlevelsCount[levelcount.key]} / <a
										href="./Problems?difficulty=${levelcount.key}">${levelcount.value}</a>)
							<c:if test="${levelcount.value!=0}">
										<fmt:formatNumber type="percent" maxIntegerDigits="3"
											value="${userProblemlevelsCount[levelcount.key]/levelcount.value}" />
									</c:if>
									<br />
								</c:forEach>
							</div>
