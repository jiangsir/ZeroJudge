						...
						<c:forEach var="problem" items="${problems}" varStatus="status">
							<tr>
								...
								<td><c:set var="problem" value="${problem}" scope="request" />
									<jsp:include page="include/div/Accepted.jsp" /></td>
								<td><c:set var="problem" value="${problem}" scope="request" />
									<jsp:include page="include/Modals/Modal_ProblemSetting.jsp" />
									<jsp:include page="include/div/ProblemTitle.jsp" /> <span
									class="pull-right"> <jsp:include
											page="include/ProblemToolbar_TypeB.jsp" /><c:if
											test="${problem.submitusers==0}">0%</c:if> <c:if
											test="${problem.submitusers>0}">
											<fmt:formatNumber
												value="${problem.acusers/problem.submitusers}"
												type="percent" />
