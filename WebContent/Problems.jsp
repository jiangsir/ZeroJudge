...
<head>
	...
	<script type="text/javascript" src="include/dialog/AdvSearch.js?${applicationScope.built }"></script>
				...
				<c:when test="${fn:length(problems)!=0}">
					<table class="table table-hover">
						<tr>
							...
							<td width="1%">&nbsp;</td>
							<td class="text-left"><fmt:message
									key="Problem.Problem" /></td>
							<td width="16%" class="text-right"><fmt:message
									key="Problem.Tags" /></td>
						...
						<c:forEach var="problem" items="${problems}" varStatus="status">
							<tr>
								...
								<td>
									...
									<jsp:include page="include/div/Accepted.jsp" />
								</td>
								<td>
									<c:set var="problem" value="${problem}" scope="request" />
									<jsp:include page="include/Modals/Modal_ProblemSetting.jsp" />
									<jsp:include page="include/div/ProblemTitle.jsp" /> <span class="pull-right">
										<jsp:include page="include/ProblemToolbar_TypeB.jsp" />
										<c:if test="${problem.submitusers==0}">0%</c:if>
										<c:if test="${problem.submitusers>0}">
											<fmt:formatNumber value="${problem.acusers/problem.submitusers}" type="percent" />
										</c:if>/<a href="./Submissions?problemid=${problem.problemid}">${problem.acusers}
								...
								<td class="text-right">
									<fmt:formatDate value="${problem.inserttime}" pattern="yyyy-MM-dd" />
								</td>
								<td class="text-right">
									<fmt:formatDate value="${problem.updatetime}" pattern="yyyy-MM-dd" />
								</td>
							</tr>
						</c:forEach>
					</table>
