...
<head>
<meta charset="UTF-8">
<c:set var="prefix" value="${problem.problemid}. ${problem.title} - "
	scope="request" />
<jsp:include page="include/CommonHead_BootstrapFlat.jsp" />

<script type="text/javascript" src="ShowProblem.js?${applicationScope.built }"></script>
<script type="text/javascript" src="include/Modals/Modal_ProblemSetting.js?${applicationScope.built }"></script>
<script type="text/javascript"
	src="include/Modals/Body/ModalBody_ProblemSetting.js?${applicationScope.built }"></script>
<script type="text/javascript" src="include/ProblemToolbar.js?${applicationScope.built }"></script>
<script type="text/javascript" src="include/div/DivProblemStatusInfo.js?${applicationScope.built }"></script>
<script type="text/javascript" src="include/div/ProblemTags.js?${applicationScope.built }"></script>
<script type="text/javascript" src="include/Modals/Modal_RejudgeProblem.js?${applicationScope.built }"></script>
...
<script src="jscripts/MathJax-3.1.4/es5/tex-mml-chtml.js" id="MathJax-script" async></script>
...
<body>
	...
	<div class="container">
		<div class="row">
			...
			<div class="col-md-4">
				<c:if test="${sessionScope.onlineUser.isProblemLevelManager() }">
					<c:set var="problemlevel" value="${problem.problemlevel}"
						scope="request" />
				題目分級：<jsp:include page="include/div/Problemlevel.jsp" /><br />
				</c:if>
				...
				: <span title="解題統計"> <a
					...
					title="不重複的嘗試解題人數">${problem.submitusers}人 </a> <c:choose>
				...
				<c:set var="problem" value="${problem}" scope="request" />
				<c:if test="${sessionScope.onlineUser.isProblemLevelManager() }">
					<jsp:include page="include/ProblemToolbar_TypeB.jsp" />
					<hr>
				</c:if>
 ...
 -->
											<pre>${fn:escapeXml(problem.sampleinputs[status.count-1])}</pre>
 ...
 -->
								...
								<div class="col-md-6">
									<div class="panel panel-default">
										...
										<div class="panel-body">
											<div class="problembox">
												<pre>${fn:escapeXml(problem.sampleoutputs[status.count-1])}</pre>
											</div>
										</div>
									</div>
								</div>
 ...
 --%>
		...
		<div class="row">
			<div class="col-md-12">
				...
				<c:if test="${!systemconfigBean.IS_CONTESTMODE && !applicationScope.appConfig.isCLASS_MODE}">
					<a href="./Submissions?problemid=${problem.problemid}" type="button" class="btn btn-success">
						<fmt:message key="Problem.Status" />
					</a>
					<a href="./Forum?problemid=${problem.problemid}" type="button" class="btn btn-success">
						<fmt:message key="Problem.Forum" />
					</a>
					<a href="./BestSolutions?problemid=${problem.problemid}" type="button" class="btn btn-success">排行</a>
					<c:if test="${sessionScope.onlineUser.getIsAccepted(problem)}">
						<a href="./NewThread?articletype=problemreport&problemid=${problem.problemid }" type="button"
							class="btn btn-primary">編寫「解題報告」</a>
					</c:if>
				</c:if>
			</div>
		</div>
							...
							<c:when test="${fn:length(reportArticles)!=0}">
								<c:forEach var="article" items="${reportArticles}"
									...
									<tr>
										...
										<td><a
											href="./ShowProblem?problemid=${article.problemid}">${article.problemid}</a></td>
										<td width="50%">
											<div align="left">
