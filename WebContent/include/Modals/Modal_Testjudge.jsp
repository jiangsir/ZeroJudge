			...
			<div class="modal-body" id="content">
				<form id="form_Testjudge">
					...
					<div class="row">
						<div class="col-md-12">
							...
							<textarea id="testcode" name="testcode" rows="10"
								class="form-control"
								style="font-family: Consolas, 'Courier New', monospace; font-size: 1.1em;"></textarea>
						</div>
					</div>
					<c:forEach var="sampleinput" items="${problem.sampleinputs}"
						...
						<div class="row">
							<div class="col-md-6">
								自訂輸入測資 #${status.count}：(可自行修改)
								<textarea id="testjudge_indata" name="testjudge_indata" rows="3"
									class="form-control">${problem.sampleinputs[status.count-1]}</textarea>
							</div>
							<div class="col-md-6">
								自訂輸出測資 #${status.count}：(可自行修改)
								<textarea id="testjudge_outdata" name="testjudge_outdata"
									rows="3" class="form-control">${problem.sampleoutputs[status.count-1]}</textarea>
							</div>
						</div>
					</c:forEach>
