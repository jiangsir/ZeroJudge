jQuery(document).ready(
	function () {
		...
		jQuery("#set100ms").click(function () {
			setTimelimit(0.1);
			recount();
		});
		jQuery("#set200ms").click(function () {
			setTimelimit(0.2);
			recount();
		});
		jQuery("#set500ms").click(function () {
			setTimelimit(0.5);
			recount();
		});
...
function setTimelimit(ms) {
	jQuery("div[id='testdataPair']").each(function () {
		$(this).find("input[name='timelimits']").val(ms);
	});
}

function averageTimelimit() {
	var total = 10; // 總時間上限
	var size = jQuery("div[id='testdataPair']").size();
	console.log(size);
	var i = 0;
	jQuery("div[id='testdataPair']").each(function () {
		var float = parseFloat($(this).find("input[name=timelimits]").val())
		console.log("float=" + float + " : " + (float == 0.0))
		if (size <= 10) {
			$(this).find("input[name='timelimits']").val("1.0");
		} else {
			var t = parseInt(100 / size) + (i < 100 % size);
			$(this).find("input[name='timelimits']").val(t / 10);
		}
		i++;
	});
}
	...
	jQuery("div[id='testdataPair']")
		.each(
			function () {
				...
				jQuery("span#totaltimelimit").html(Math.round(totaltimelimit*10)/10);
			});
