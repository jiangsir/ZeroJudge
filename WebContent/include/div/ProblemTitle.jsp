...
<c:choose>
	<c:when test="${problem.display == problem.DISPLAY_OPEN}">
		<!-- 		<span class="glyphicon glyphicon-globe" title="公開題"></span>
 -->	   ${problem.problemid}. <a
	...
	<c:when test="${problem.display == problem.DISPLAY_PRACTICE}">
		...
		<span class="glyphicon glyphicon-eye-close" title="練習題"></span>	
	${problem.problemid}. <a
			href="./ShowProblem?problemid=${problem.problemid}"
			title="${problem.backgrounds}">${fn:escapeXml(problem.title)}</a>
	</c:when>
	<c:otherwise>
		<span style="text-decoration: line-through;" title="隱藏題">
			${problem.problemid}. <a
			href="./ShowProblem?problemid=${problem.problemid}"
			title="${problem.backgrounds}">${fn:escapeXml(problem.title)}</a>
		</span>
