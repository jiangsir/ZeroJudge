...
<div class="container-fluid">
	...
	<div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
		<c:choose>
			<c:when test="${applicationScope.appConfig.isCONTEST_MODE()}">
				<ul class="nav navbar-nav">
					<li><a href="${pageContext.request.contextPath}/Contests">
							<div id="banner">
								<div class="FirstPageTitle" title="競賽模式">
									${applicationScope.appConfig.header}
								</div>
							</div>
						</a>
					</li>
			...
			<c:when test="${applicationScope.appConfig.isCLASS_MODE()}">
				<ul class="nav navbar-nav">
					<li><a href="${pageContext.request.contextPath}/ShowVClasses">
							<div id="banner">
								<div class="FirstPageTitle" title="課程模式">
									${applicationScope.appConfig.header}
								</div>
							</div>
						</a>
			...
			<c:otherwise>
				<ul class="nav navbar-nav">
					...
					<li><a href="${pageContext.request.contextPath}/Forum">
							<fmt:message key="Header.Forum" />
						</a></li>
					<li><a href="${pageContext.request.contextPath}/Contests">
				...
				<form name="form1" class="navbar-form navbar-center" role="search" method="POST"
					action="${pageContext.request.contextPath}/Problems">
		...
		<ul class="nav navbar-nav navbar-right">
			<%-- <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button"
					aria-expanded="false">
					<fmt:message key="LANGUAGE" />(${sessionScope.session_locale}) <span class="caret"></span>
				</a>
				<ul class="dropdown-menu" role="menu">
					<li><a href="./ChangeLocale.api?locale=zh_TW">
							<fmt:message key="zh_TW" />
						</a></li>
					<li><a href="./ChangeLocale.api?locale=zh_CN">
							<fmt:message key="zh_CN" />
						</a></li>
					<li><a href="./ChangeLocale.api?locale=en_US">
		...
		<ul class="nav navbar-nav navbar-right">
			<%-- <li><a>
					<fmt:message key="ONLINE_USERS" />:
					${fn:length(applicationScope.onlineUsers)} |
					<fmt:message key="PROBLEM_NUM" />:
					${fn:length(applicationScope.openedProblemidSet)}
				</a></li> --%>
				<c:choose>
					<c:when test="${sessionScope.onlineUser != null && !sessionScope.onlineUser.nullUser}">
						<%-- <c:if test="${user.isAdmin(sessionScope.currentUser)}">
							<li><a href="./EditAppConfig"><span class="glyphicon glyphicon-wrench"
							...
							<li class="dropdown"><a href="#" class="dropdown-toggle " data-toggle="dropdown" role="button"
									...
									<c:if test="${sessionScope.onlineUser.isAuthhost_Google }">
										<img src="${pageContext.request.contextPath}/ShowImage?userid=${sessionScope.onlineUser.id}"
								...
								<ul class="dropdown-menu" role="menu">
									<c:if test="${applicationScope.appConfig.isGoogleLoginSetup }">
										<c:if test="${!sessionScope.onlineUser.isAuthhost_Google }">
											<li><a href="${pageContext.request.contextPath}/GoogleLogin"><i
														class="fa fa-link" aria-hidden="true"></i> 綁定 Google 帳號</a></li>
										</c:if>
										<c:if test="${sessionScope.onlineUser.isAuthhost_Google}">
											<li><img src="${pageContext.request.contextPath}/ShowImage?userid=${sessionScope.onlineUser.id}"
													class="img-responsive" style="height: 200px;"></li>
											<li><a href="#" type="button" data-toggle="modal"
													data-target="#Modal_SetUserPassword"><i class="fa fa-chain-broken"
														aria-hidden="true"></i> 解除綁定
													Google 帳號</a></li>
										</c:if>
									</c:if>
									...
									<c:if
										...
										<li><a href="${pageContext.request.contextPath}/EditContests"><i
													class="fa fa-certificate"></i> 競賽管理</a></li>
									</c:if>
									<c:if test="${sessionScope.onlineUser.problemManager}">
										<li><a href="${pageContext.request.contextPath}/EditProblems"><i
													class="fa fa-certificate"></i> 題目管理</a></li>
									</c:if>
									<c:if test="${sessionScope.onlineUser.VClassManager}">
										<li><a href="${pageContext.request.contextPath}/ShowVClasses"><i
									...
									<c:if test="${sessionScope.onlineUser.isHigherEqualThanMANAGER}">
										...
										<li><a href="${pageContext.request.contextPath}/EditSchools"><i
													class="fa fa-certificate"></i> 管理學校</a></li>
										<li><a href="${pageContext.request.contextPath}/EditAppConfig"><i
									...
									<c:if test="${!applicationScope.appConfig.getIsCLASS_MODE() }">
										<li><a href="${pageContext.request.contextPath}/ShowIMessages?action=inboxIM">
												<fmt:message key="Header.IMessageInbox" />
											</a></li>
										<li><a href="${pageContext.request.contextPath}/UserStatistic">
												<fmt:message key="Header.UserStatistic" />
											</a></li>
									</c:if>
									...
									<c:if test="${fn:length(sessionScope.onlineUser.belongedVClasses) > 0}">
										<li>
									...
									<%-- <c:if test="${sessionScope.onlineUser.isDEBUGGER}">
										...
										<c:if test="${sessionScope.onlineUser.isDEBUGGER}">
											<li><a href="#">Debug</a>
												<ul>
													<li><a href="${pageContext.request.contextPath}/Debug">Debug資訊看版</a>
													</li>
													<li><a
															href="${pageContext.request.contextPath}/ShowOnlineUsers">管理線上使用者</a>
													</li>
