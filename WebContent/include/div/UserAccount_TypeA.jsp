...
<c:if test="${!applicationScope.appConfig.isCLASS_MODE}">
	<c:if test="${user.isEnabled==true}">
		<span style="display: inline-block;">
			<c:if test="${user.isAuthhost_Google && fn:length(user.pictureblob)>0 }">
				<img src="./ShowImage?userid=${user.id}" class="img-rounded" style="width: 1.4em;"
					title="${user.email }">
			</c:if>
			<span><a href="./UserStatistic?id=${user.id}" title="${user.account}">${fn:substring(user.account, 0, 12)}<c:if test="${fn:length(user.account)>12}">...</c:if></a></span><span title="${fn:escapeXml(user.username)}">(${fn:substring(fn:escapeXml(user.username), 0, 15)}<c:if test="${fn:length(fn:escapeXml(user.username))>15}">...</c:if>)</span>
