...
<span id="statusinfo" data-solutionid="${solution.id}" data-problemscores="${solution.problem.scoresByJSON }"> <img
		src="images/Spinner.gif" id="Spinner" style="display: none;" /> <span id="judgement"
		data-solutionid="${solution.id }">
		<c:if test="${sessionScope.onlineUser.canShowDetails(solution) }">
			<c:choose>
				<c:when test="${solution.isJudgement_AC }">
					<a href="#" class="acstyle" data-solutionid="${solution.id}">${solution.judgement }</a>
				</c:when>
	...
	<c:if test="${sessionScope.onlineUser.isRejudgable(solution)}">
		<div class="btn btn-default btn-xs" id="rejudgeSolution" data-solutionid="${solution.id }">
