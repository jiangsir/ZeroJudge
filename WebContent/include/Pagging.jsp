 ...
 --%>
	<c:choose>
		<c:when test="${pagenum!=1}">
			<a href="?page=1&${param.querystring}"><fmt:message
					key="FIRST_PAGE" /></a>
		</c:when>
		<c:otherwise>
	...
	<c:choose>
		<c:when test="${pagenum>1}">
			《- <a href="?page=${pagenum-1}&${param.querystring}"><fmt:message
					key="PREV_PAGE" /></a>
		</c:when>
		<c:otherwise>
	...
	<c:choose>
		<c:when
			...
			<a href="?page=${pagenum+1}&${param.querystring}"
	...
	<c:if test="${not empty lastpage && lastpage>1 && pagenum<lastpage}">
		| <a href="?page=${lastpage}&${param.querystring}"><fmt:message
				key="LAST_PAGE" /></a>
	</c:if>
