...
<body>
  ...
  -->
			<table class="table table-hover">
				...
				<c:forEach var="contestant" items="${contestants}" varStatus="varstatus">
					<tr>
						<td title="contestant ID">#${contestant.id}<br>
							<c:if test="${contestant.isOnline}">
						...
						<td id="account">#${varstatus.count}:
							<c:set var="user" value="${contestant.user}" scope="request" />
							<jsp:include page="include/div/UserAccount_TypeA.jsp" />
							<c:if test="${contestant.school!='' }">
								<br />${fn:escapeXml(contestant.school)}
							</c:if>
						</td>
						<td>${fn:escapeXml(contestant.teamaccount)}</td>
						...
						<td>
							...
							<c:if test="${contestant.isRejoin }">
								<div class="btn-group btn-group-sm" role="group" aria-label="...">
									<button type="button" class="btn btn-default" data-target="#Modal_confirm"
										title="恢復使用者/重新加入" data-title="恢復使用者/重新加入"
										data-content="恢復 ${contestant.user.account} 將可以重新加入，確定嗎？" data-type="POST"
						...
						<td width="20%">
							<div class="btn-group btn-group-sm" role="group" aria-label="...">
								<button type="button" class="btn btn-default" data-target="#Modal_confirm"
									title="將使用者踢出" data-title="將使用者踢出"
								...
								<button type="button" class="btn btn-default" data-target="#Modal_confirm" title="立即交卷"
									data-title="立即交卷" data-content="使用者將無法再繼續解題。確定讓 ${contestant.user.account} 強制交卷？"
									data-type="POST" data-url="Contest.api"
