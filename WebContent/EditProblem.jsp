...
<script type="text/javascript">
	jQuery(document).ready(function () {
		var problemid = $("input[id='problemid']").val();
		console.log("tinymce6 image problemid=" + problemid);
		var images_upload_url = "UploadImages.api?action=uploadProblemImage&problemid=" + problemid;
...
<body>
	...
	<div class="container-fluid">
		<a href="EditProblems" class="btn btn-primary btn-lg">回題目管理</a> <a
			href="EditProblems?tabid=${problem.tabid}#${problem.tabid}" class="btn btn-primary btn-lg">管理
			${problem.tab}</a>
			<hr>
		<div class="row">
			<div class="col-md-6">
				<form id="updateProblem" name="updateProblem" method="post" action="">
					<input name="problemid" type="hidden" id="problemid" value="${problem.problemid}" />
					<fieldset>
						<legend>
							<a href="./ShowProblem?problemid=${problem.problemid}">題目
								(${problem.problemid})</a>
							<jsp:include page="include/div/ProblemOwner.jsp" />
						</legend>
						<div class="form-group">
							<label for="title">題目標題</label> <input type="text" name="title" class="form-control" placeholder="標題"
								value="${fn:escapeXml(problem.title)}">
						</div>
			
						<div class="form-group">
							<label for="content">內容：</label>
							<textarea name="content" class="mceImage"
								rows="20">${fn:replace(problem.content, "&", "&amp;")}</textarea>
						</div>
			
						<div class="form-group">
							<label for="theinput">輸入說明：</label>
							<textarea name="theinput" class="mceImage"
						...
						<div class="form-group">
							...
							<textarea name="theoutput" class="mceImage"
								rows="5">${fn:replace(problem.theoutput, "&", "&amp;")}</textarea>
						</div>
						<c:forEach var="sampleinput" items="${problem.sampleinputs}" varStatus="status">
							<div class="row" id="SampleInputOutput">
								<div class="col-md-6">
									<div class="form-group">
										<label for="sampleinput">
											<fmt:message key="Problem.SampleInput" /><span id="sample_sn">#${status.count}</span>:
										</label>
										<textarea name="sampleinput" class="form-control" rows="3"
											style="font-family: Consolas, 'Courier New', monospace;">${fn:escapeXml(problem.sampleinputs[status.count-1])}</textarea>
									</div>
								</div>
								<div class="col-md-6">
									<div class="form-group">
										<div class="btn btn-default btn-xs pull-right" id="removeSample" title="刪除本範例測資">
											<span class="fa fa-times"></span>
										</div>
										<label for="sampleoutput">
											<fmt:message key="Problem.SampleOutput" /><span id="sample_sn">#${status.count}</span>:
										</label>
										<textarea name="sampleoutput" class="form-control" rows="3"
											style="font-family: Consolas, 'Courier New', monospace;">${fn:escapeXml(problem.sampleoutputs[status.count-1])}</textarea>
									</div>
								</div>
							</div>
						</c:forEach>
						...
						<div class="form-group">
							...
							<textarea name="hint" class="form-control mceImage" rows="3">${problem.hint}</textarea>
						</div>
						...
						<div class="form-group">
							...
							<textarea name="samplecode" class="form-control" rows="10"
								style="font-family: Consolas, 'Courier New', monospace;">${problem.samplecode}</textarea>
						</div>
						<div class="form-group">
							<label for="comment">備註：</label>
							<textarea name="comment" class="form-control" rows="5" id="comment">${problem.comment}</textarea>
						</div>
						<br /> <br /> <input name="tab" type="hidden" id="tab" value="tab10,原創題目" /> <input name="pid" type="hidden"
							id="pid" value="${problem.id}" /> <input name="problemid" type="hidden" id="problemid"
							value="${problem.problemid}" />
						<input name="sortable" type="hidden" value="${problem.sortable}" /> <input name="keywords" type="hidden"
							value="${problem.keywords}" /> <input type="submit" name="updateProblem" value="儲存題目內容"
				...
				<div class="row">
					<div class="col-md-12">
						<table class="table-hover">
							<tr>
								<td>
									<jsp:include page="include/ProblemToolbar.jsp" />
								</td>
							</tr>
						</table>
					</div>
				...
				<div class="row">
					<div class="col-md-12" id="ProblemSettings">
						<table class="table-hover">
							<tr>
								<td>
									<jsp:include page="include/Modals/Body/ModalBody_ProblemSetting.jsp" /> <input
										type="button" class="btn btn-success btn-lg col-md-12" id="save_ProblemSettings"
										value="儲存設定" data-problemid="${problem.problemid }" />
								</td>
							</tr>
						</table>
						<hr>
					...
					<div class="col-md-12">
						<table class="table-hover">
							<tr>
								<td>
									<h2>上傳測資檔</h2>
									<form id="uploadTestdatas" action="" method="post" enctype="multipart/form-data"
										name="uploadTestdatas" id="uploadTestdatas">
										<div>
											上傳測資檔需符合以下規則:<br>
											<ul>
										...
										<div>
											...
											filename_##.out) <br /> <input type="file" multiple="multiple"
												name="testdatas" /> (請複選) <br />
										</div>

										<button type="button" class="btn btn-success btn-lg col-md-12" id="uploadFiles"
					...
					<div class="col-md-12">
						<table class="col-md-12 table-hover">
							<tr>
								<td>
									<h2>測資配分、配時</h2>
									<jsp:include page="include/div/TestdataPairs.jsp" />
								</td>
							</tr>
						</table>
						<hr />
														...
														<div>
															...
															<script type="text/javascript">
																jQuery(document).ready(function () {
																	...
																	$('input[type=radio][name=specialjudge_language]').change(function () {
															...
															<textarea id="specialjudge_code" name="specialjudge_code"
																cols="90%" rows="20"
																style="font-family: Consolas, 'Courier New', monospace;">${problem.specialjudge_code}</textarea>
															<div>
