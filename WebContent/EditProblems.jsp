...
<!-- <script type="text/javascript" src="showDetails.js"></script>
 -->
<script type="text/javascript" src="./jscripts/nav_tabs_show.js"></script>
...
<body>
	...
	<div class="container">
		<div class="row">
			<form name="form1" class="navbar-form navbar-center" role="search"
				method="POST" action="./EditProblems">
			...
			<c:if test="${sessionScope.onlineUser.isDEBUGGER}">
				...
				<span id="advsearch" class="btn btn-primary">進階搜尋</span>
			</c:if>

			<c:if test="${sessionScope.onlineUser.isDEBUGGER}">
				<span class="DEBUGGEROnly"> <jsp:include
 ...
 -->
			...
			<ul class="nav nav-tabs" role="tablist">
				<li><a href="?tabid=MYPROBLEM#aria_MYPROBLEM"
					aria-controls="aria_MYPROBLEM" role="presentation">我的所有題目</a></li>
				<c:forEach var="tab"
 ...
 --%>
					<li><a href="?tabid=${tab.id}#aria_${tab.id}"
						aria-controls="aria_${tab.id}" role="presentation">${tab.name}</a></li>

				</c:forEach>

				<c:if test="${sessionScope.onlineUser.isDEBUGGER}">
					<li class="DEBUGGEROnly"><a
						href="?tabid=VERIFYING#aria_VERIFYING"
						aria-controls="aria_VERIFYING" role="presentation">已提交</a></li>
				</c:if>
				<li><a href="?tabid=NOTOPEN#aria_NOTOPEN"
					aria-controls="aria_NOTOPEN" role="presentation"><span
						class="glyphicon glyphicon-eye-close"></span> 非開放題</a></li>
				<li><a href="?tabid=SPECIAL#aria_SPECIAL"
					aria-controls="aria_SPECIAL" role="presentation">Special</a></li>
			</ul>
				...
				<c:choose>
					<c:when test="${fn:length(problems)!=0}">
						<c:forEach var="problem" items="${problems}" varStatus="varstatus">
							<tr data-problemid="${problem.problemid}">
...
<%-- 								<td id="problemid">${problem.problemid}</td>
 --%>								<td><c:set var="problem" value="${problem}" scope="request" />
									...
									<span class="pull-right"><jsp:include
										page="include/div/ProblemDifficulty.jsp" /></span>
								</td>
								<td>${fn:escapeXml(problem.tags)}</td>
								<td><c:set var="problem" value="${problem}" scope="request" />
									<jsp:include page="include/ProblemToolbar.jsp" /></td>
							</tr>
						</c:forEach>
					</c:when>
					<c:otherwise>
