...
<body>
	...
	<div class="container" id="alert">
		<div class="row">
			<fieldset>
				<legend id="type">${alert.type}</legend>
				<h1 id="title">${alert.title}</h1>
				<p></p>
				<pre id="subtitle">${alert.subtitle }</pre>
				<hr>
				<c:if test="${alert.content!=''}">
					<div id="content">${alert.content}</div>
					<hr>
				</c:if>
				<ul id="list">
					...
					<c:if test="${fn:length(alert.stacktrace)>0}">
						<div style="text-align: left; margin-top: 1em;">
							...
							<div style="font-family: Consolas, monospace;">
								<c:forEach var="stacktrace" items="${alert.stacktrace}">
									${stacktrace.className}.${stacktrace.methodName}(${stacktrace.fileName}:${stacktrace.lineNumber})<br />
								</c:forEach>
							</div>
						</div>
					</c:if>
