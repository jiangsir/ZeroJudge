...
<script type="text/javascript"
	src="EditContest.js?${applicationScope.built }"></script>
<script type="text/javascript"
...
<body>
		...
		<div class="row">
			<form name="form" method="post" action="">
				<h1>
					...
					<%-- 
					<c:choose>
						<c:when test="${contest.vclassid>0}">編輯『<fmt:message key="Contest.VContest" />』</c:when>
						<c:otherwise>編輯『<fmt:message key="Contest.Contest" />』</c:otherwise>
					</c:choose>
 --%>
				</h1>
							...
							<div class="form-group">
								...
								<div class="input-group">
									<input type="text" class="form-control"
										value="${contest.problemids}" name="problemids"
										id="problemids" aria-describedby="problemids_help">
							...
							<div class="form-group">
								...
								<div class="input-group">
									<input type="text" class="form-control"
										...
										aria-describedby="scores_help">
									<div class="btn btn-default input-group-addon" id="doscore100">配100</div>
									<div class="btn btn-default input-group-addon"
										id="doaveragescore">平均配分</div>
									<div class="input-group-addon">
										共 <span id="totalscore">0</span> 分
									</div>
								</div>
								<span id="scores_help" class="help-block">配分欄位以逗點相隔，可任意配分，總和不一定需要為100，可依需求任意指定，惟須確保為&gt;0的正整數，如
									[40,60]</span>
 ...
 --%>
							<h4>
								*
								<jsp:include page="include/div/Bundle_Contest.jsp" />進行中之相關設定：
							</h4> <c:if test="${contest.VContest}">
								<div>
							...
							<div>
								...
								是 <input id="config" name="config_${contest.config_ShowDetail}"
									type="radio" value="0" data-contestconfig="${contest.config}" />
								否] 允許參賽者查看詳細錯誤狀況？<br /> *「是」則可以顯示詳細情況，可以查看 NA 內的各種錯誤。<br />
							...
							<div>
								[ <input id="config"
									name="config_${contest.config_ShowOnlyOneContestant}"
									type="radio" value="1" data-contestconfig="${contest.config}" />
								是 <input id="config"
									name="config_${contest.config_ShowOnlyOneContestant}"
									type="radio" value="0" data-contestconfig="${contest.config}" />
								否] 是：僅顯示參與者個人的成績。否：列出所有參與者。
							</div>
							<h4>* 評審設定：</h4>
							<div>
								[ <input id="config" name="config_${contest.config_ManualJudge}"
									type="radio" value="1" data-contestconfig="${contest.config}" />
								是 <input id="config" name="config_${contest.config_ManualJudge}"
									type="radio" value="0" data-contestconfig="${contest.config}" />
							...
							<div>
								...
								是 <input id="config" name="config_${contest.config_Visible}"
									type="radio" value="0" data-contestconfig="${contest.config}" />
								否]
								<jsp:include page="include/div/Bundle_Contest.jsp" />結束後，是否讓他人參觀本次<jsp:include
									page="include/div/Bundle_Contest.jsp" />題目？
							</div>
							<div>
								[ <input id="config"
									...
									type="radio" value="0" data-contestconfig="${contest.config}" />
								否]
								<jsp:include page="include/div/Bundle_Contest.jsp" />結束後，立即公開<jsp:include
									page="include/div/Bundle_Contest.jsp" />狀態
							</div>
							<div>
								[ <input id="config"
								...
								否]
								<jsp:include page="include/div/Bundle_Contest.jsp" />結束後，立即公開<jsp:include
									page="include/div/Bundle_Contest.jsp" />結果
							</div>
						</td>
					</tr>
							...
							<td>
								...
								<div>
									<input name="contestvisible" type="radio" value="open"
										...
										data-contestvisible="${contest.visible}" /> 隱藏(<jsp:include
										page="include/div/Bundle_Contest.jsp" />) <br /> <br />
									額外的權限控制：(未完成) <input name="addonprivilege" type="text"
