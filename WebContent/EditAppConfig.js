...
jQuery(document).ready(function () {
	...
	$("button[id='checkServerConfig']").on("click", function () {
		var step1 = $("span[id='checkAllowedIP']");
		checkServerConnection_AllowedIP(step1);
		var step2 = $("span[id='checkReadServerConfig']");
		checkServerConnection_ReadServerConfig(step2);
		var step3 = $("span[id='checkNopassRsync']");
		checkServerConnection_Nopassrsync(step3);
		var step4 = $("span[id='checkLocker']");
		checkServerConnection_Locker(step4);
	});
	...
	jQuery("input[name=enableMailer]").each(function () {
		if ($(this).data("enablemailer")) {
			$(this).attr("checked", "true");
		} else {
			$(this).attr("checked", "false");
		}

		// var enableMailer = $(this).data("enablemailer");
		// if (enableMailer == "true") {
		// $(this).attr("checked", "true");
		// } else {
		// $(this).attr("checked", "false");
		// }
		click_enableMailer($(this));
	});

	jQuery("input[name=enableMailer]").click(function () {
		click_enableMailer($(this));
	});

	jQuery("input[name=enableReCaptcha]").each(function () {
		console.log("data:enableReCaptcha=" + $(this).data("enable_recaptcha"));
		console.log("attr:enableReCaptcha=" + $(this).attr("data-enable_recaptcha"));

		if ($(this).data("enable_recaptcha")) {
			$(this).attr("checked", true);
		} else {
			$(this).attr("checked", false);
		}
		click_enableReCaptcha($(this));
	});
	jQuery("input[name=enableReCaptcha]").click(function () {
		click_enableReCaptcha($(this));
	});

	jQuery("input[name=enableGoogleLogin]").each(function () {
		var enableGoogleLogin = $(this).attr("data-enableGoogleLogin");
		if (enableGoogleLogin == "true") {
			$(this).prop("checked", true);
		} else {
			$(this).prop("checked", false);
		}
		click_enableGoogleLogin($(this));
	});
	jQuery("input[name=enableGoogleLogin]").click(function () {
		click_enableGoogleLogin($(this));
	});
	...
	jQuery("#addTab").click(function () {
		...
		console.log("jQuery(tr[id=tab]).size()=" + jQuery("tr[id=tab]").size() + ", jQuery(tr[id=newTab]).size()=" + jQuery("tr[id=newTab]").size());
		if (i < 10) {
	...
	jQuery("a#deleteTab").click(function (event) {
			...
			buttons: {
				"確定": function () {
					jQuery.ajax({
						type: "GET",
						url: "Debugger.api",
						data: "action=deleteProblemtab&tabid=" + tabid + "&renametotabid=" + delete_Confirm.find("input[name='renametotabid']:checked").val(),
						async: false,
						timeout: 5000,
	...
	jQuery("#problemid_prefix").children().each(function () {
		...
		if ($(this).parent().data("problemid_prefix") == $(this).val()) {
			$(this).attr("selected", true);
		}
	});
	...
	// }
	// });
	jQuery("input[name='registerable']").each(function () {
		if ($(this).val() == $(this).data("registerable").toString()) {
			console.log("!!!!! registeralbe: 為 boolean 必須 toString()")
			$(this).attr("checked", true);
		}
	});
	jQuery("input[name='rejudgeable']").each(function () {
		if ($(this).val() == $(this).data("rejudgeable").toString()) {
			console.log("!!!!! rejudgealbe: 為 boolean 必須 toString()")
			$(this).attr("checked", true);
		}
	});
	jQuery("input[name='applyTeacher']").each(function () {
		if ($(this).val() == $(this).data("applyteacher").toString()) {
			console.log("!!!!! applyTeacher: 為 boolean 必須 toString()");
			$(this).attr("checked", true);
		}
	});
	jQuery("input[name='applyProblemManager']").each(function () {
		if ($(this).val() == $(this).data("applyproblemmanager").toString()) {
			console.log("!!!!! applyProblemManager: 為 boolean 必須 toString()");
			$(this).attr("checked", true);
		}
	});
	jQuery("input[name='systemMode']").each(function () {
		console.log("!!!!systemmode:" + $(this).data("systemmode"));
		if ($(this).data("systemmode") == $(this).val()) {
			$(this).attr("checked", true);
		}
	});
...
function click_enableMailer(enableMailer) {
	if (enableMailer.is(":checked")) {
		enableMailer.prop("checked", true);
		enableMailer.val(true);
		enableMailer.closest(".form-group").find("input[type=text]").prop('disabled', false);
		// enableReCaptcha.closest(".form-group").find("input[name=site_key]").prop('disabled',
		// false);
		// enableReCaptcha.closest(".form-group").find("input[name=secret_key]").prop('disabled',
		// false);
	} else {
		enableMailer.prop("checked", false);
		enableMailer.val(false);
		enableMailer.closest(".form-group").find("input[type=text]").prop('disabled', true);
		// enableReCaptcha.closest(".form-group").find("input[name=site_key]").prop('disabled',
		// true);
		// enableReCaptcha.closest(".form-group").find("input[name=secret_key]").prop('disabled',
		// true);
...
function click_enableReCaptcha(enableReCaptcha) {
	if (enableReCaptcha.is(":checked")) {
		enableReCaptcha.prop("checked", true);
		enableReCaptcha.val(true);
		enableReCaptcha.closest(".form-group").find("input[type=text]").prop('disabled', false);
		// enableReCaptcha.closest(".form-group").find("input[name=site_key]").prop('disabled',
		// false);
		// enableReCaptcha.closest(".form-group").find("input[name=secret_key]").prop('disabled',
		// false);
	} else {
		enableReCaptcha.prop("checked", false);
		enableReCaptcha.val(false);
		enableReCaptcha.closest(".form-group").find("input[type=text]").prop('disabled', true);
		// enableReCaptcha.closest(".form-group").find("input[name=site_key]").prop('disabled',
		// true);
		// enableReCaptcha.closest(".form-group").find("input[name=secret_key]").prop('disabled',
		// true);
...
function click_enableGoogleLogin(enableGoogleLogin) {
	if (enableGoogleLogin.is(":checked")) {
		enableGoogleLogin.prop("checked", true);
		enableGoogleLogin.val(true)
		enableGoogleLogin.closest(".form-group").find("input[type=text]").prop('disabled', false);
		// enableGoogleLogin.closest(".form-group").find("input[name=client_id]").prop('disabled',
		// false);
		// enableGoogleLogin.closest(".form-group").find("input[name=client_secret]").prop('disabled',
		// false);
	} else {
		enableGoogleLogin.prop("checked", false);
		enableGoogleLogin.val(false)
		enableGoogleLogin.closest(".form-group").find("input[type=text]").prop('disabled', true);
		// enableGoogleLogin.closest(".form-group").find("input[name=client_id]").prop('disabled',
		// true);
		// enableGoogleLogin.closest(".form-group").find("input[name=client_secret]").prop('disabled',
		// true);
