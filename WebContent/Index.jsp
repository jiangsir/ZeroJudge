				...
				<div class="row">
					<div class="col-md-8">
						...
						<a href="./UserGuide.html#compiler">
							<fmt:message key="Index.AboutCompiler" />
						</a><br />
						...
						<div class="row">
							<div class="col-md-12">
								<a href="UserGuide.html" type="button" class="btn btn-default">
									<fmt:message key="Index.UserGuide" />
								</a>
								<jsp:include page="include/Modals/Modal_applyProblemManager.jsp" />
								<!-- 								<a href="./Application" type="button" class="btn btn-default">我要加題目</a>
 -->
								<c:if
									test="${sessionScope.isOnlineUser && applicationScope.appConfig.isChecked_applyProblemManager()}">
									<button type="button" class="btn btn-primary" data-toggle="modal"
										data-target="#Modal_applyProblemManager">我要加題目</button>
								</c:if>
								<c:if
									test="${sessionScope.isOnlineUser && applicationScope.appConfig.isChecked_applyTeacher()}">
									<!-- <a href="#" type="button" class="btn btn-default">我要開課</a> -->
									<jsp:include page="include/Modals/Modal_applyTeacher.jsp" />

									<button type="button" class="btn btn-primary" data-toggle="modal"
										data-target="#Modal_applyTeacher">我要開課</button>
								</c:if>
								<c:if test="${fn:length(applicationScope.appConfig.httpscrt)!=0 }">
									<a href="./Download.api?target=HttpscrtFile" type="button"
										class="btn btn-default">下載憑證檔</a>
								</c:if>
							</div>
						</div>
					</div>
					<div class="col-md-4">
						<div class="row">
							<c:choose>
								<c:when test="${fn:length(startingContests)>0 || fn:length(runningContests)>0}">
									<div>
										<c:forEach var="contest" items="${runningContests}">
											<c:set var="contest" value="${contest}" scope="request" />
											<jsp:include page="include/ContestHeader_TypeB.jsp" />
										</c:forEach>
									...
									<tr>
										<td><i class="fa fa-circle-o" aria-hidden="true"></i> <a
											...
											<span style="font-size: small; color: #AAAAAA"> (
												<fmt:formatDate value="${announcement.timestamp}" pattern="yyyy-MM-dd HH:mm" />)
											</span>
										</td>
									</tr>
								</c:forEach>
					...
					<div class="col-md-12">
						<table class="table table-hover table-condensed">
							<thead>
								<tr>
									<th>
										<fmt:message key="Index.TopCoder" />
									</th>
									<th></th>
								</tr>
							</thead>
								...
								<c:choose>
									<c:when test="${fn:length(top10)!=0}">
										<c:forEach var="top10" items="${top10}" varStatus="varstatus">
											<tr>
												<td>
													<c:set var="user" value="${top10}" scope="request" />
													<jsp:include page="include/div/UserAccount_TypeA.jsp" />
												</td>
												<td title="通過題數 / 點數" style="width: 30%; font-size: xx-small;">${top10.ac}/
													<fmt:formatNumber value="${top10.ac/fn:length(applicationScope.openedProblemidSet)}" type="percent" />
												</td>
											</tr>
										</c:forEach>
										<tr>
							...
							<tbody>
								<c:choose>
									<c:when test="${fn:length(topauthors)!=0}">
										<c:forEach var="topauthor" items="${topauthors}" varStatus="varstatus">
											<tr>
												<td>
													<c:set var="user" value="${topauthor.key}" scope="request" />
													<jsp:include page="include/div/UserAccount_TypeA.jsp" />
													<%-- <a href="./UserStatistic?account=${topauthor.key}"
														title="${topauthor.key}">${topauthor.key}</a> --%>
												</td>
												<td title="">
													<div align="right">
														<form id="searchAuthor"
															action="Problems?ownerid=${topauthor.key.id}" method="post"
															class="btn btn-link btn-xs">${topauthor.value}</form>
													</div>
												</td>
											</tr>
										</c:forEach>
							...
							<tbody>
								<c:choose>
									<c:when test="${fn:length(topschools)!=0}">
										<c:forEach var="school" items="${topschools}" varStatus="varstatus">
											<tr>
												<td>${varstatus.count}:
