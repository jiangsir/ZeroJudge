jQuery(document).ready(function() {
	...
	jQuery("div#newSampleInputOutout").click(function(e) {
		var form = $(this).closest("form");
		var sample = form.find('div[id="SampleInputOutput"]:last');
		var clone = sample.clone(true);
		clone.find("input, textarea").val("");
		clone.find("div[id='removeSample']").removeClass('hidden');
		clone.insertAfter(sample);
		var index = 0;
		form.find('div[id="SampleInputOutput"]').each(function() {
			$(this).find("span[id='sample_sn']").text('#' + ++index);
		});
	});

	jQuery("div#removeSample").click(function(e) {
		var sampleIO = $(this).closest("#SampleInputOutput");
		console.log(jQuery("div[id=SampleInputOutput]").size());
		if (jQuery("div[id=SampleInputOutput]").size() > 1) {
			sampleIO.remove();
		}
	});
	...
	jQuery("input[name='saveUploadSpecialJudgeFiles']").click(function(e) {
		...
		jQuery.ajax({
			...
			error : function(jqXHR, textStatus, errorThrown) {
				console.log("ajaxerror: jqXHR.responseText=" + jqXHR.responseText);
				...
				try {
					...
					$textAndPic.append(alert1.title + '<br/>' + alert1.onlineUser.account);
					if (alert1.debugs != '') {
						$textAndPic.append('LoopbackDebugger=' + alert1.debugs + '<br/>');
					}
					...
					BootstrapDialog.alert(errorThrown + ", saveUploadSpecialJudgeFiles url=" + jqXHR.url);
				}
			}
		});
	});
	...
	jQuery("button[id='uploadFiles']").click(function(e) {
		// this.closest("form").submit();
		e.preventDefault();
		var form = $(this).closest("form");
		// console.log("form=" + form);
		var problemid = $(this).data("problemid");
		var timeoutt = 300 * 1000; // 測資上傳時限
		...
		jQuery.ajax({
			...
			beforeSend : function(jqXHR, settings) {
				...
				uploadbutton.data("loading-text", "Uploading...");
			...
			error : function(jqXHR, textStatus, errorThrown) {
				...
				try {
					...
					uploadbutton.button('reset');
					BootstrapDialog.alert("讀取錯誤! " + errorThrown + "(" + timeoutt + "),uploadFiles url=" + jqXHR.url);
				}
			}
		});
	...
	// console.log('updateProblem click!!!')
