			...
			<c:choose>
				<c:when
					...
					<c:choose>
						<c:when
							...
							<h2>
								您已經參加[<a
									href="./ShowContest?contestid=${sessionScope.onlineUser.joinedcontestid}">${sessionScope.onlineUser.joinedContest.title}</a>]當中，請交卷後再參加本競賽。
							</h2>
						</c:when>
						<c:otherwise>
				...
				<c:when
					...
					<button type="button" class="btn btn-success" data-toggle="modal"
						...
						data-content="交卷後就無法再進入 「${contest.title}」了，確定嗎?" data-type="POST"
