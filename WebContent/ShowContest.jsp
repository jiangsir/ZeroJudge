				...
				<c:when
					...
					<c:choose>
						<c:when
							...
							<h2>
								您已經參加[<a
									href="./ShowContest?contestid=${sessionScope.onlineUser.joinedcontestid}">${sessionScope.onlineUser.joinedContest.title}</a>]當中，請交卷後再參加。
							</h2>
						</c:when>
						<c:otherwise>
							<h2>參加後才會顯示此處的資訊！!!</h2>
