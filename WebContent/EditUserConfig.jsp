...
<head>
	...
	<script type="text/javascript" src="EditUserConfig.js?${applicationScope.built}"></script>
		...
		<div class="row">
			...
			<form class="form-horizontal">
				<table class="table table-hover">
					...
					<tr>
						<td>備註：</td>
						<td><textarea name="comment" cols="50" rows="3" id="comment">${fn:escapeXml(user.comment)}</textarea></td>
					</tr>
					<tr>
						<td><div id="userconfig" data-userconfig="${user.config }">使用者參數：</div></td>
						<td>
							<!-- <input name="config" type="checkbox" id="$ContestManager"
							value="2" />#2 「競賽/隨堂練習管理員」的權限<br /> <input name="config"
							type="checkbox" id="InsertProblem" value="3" />#3 新增題目的權限
							(InsertProblem) <br /> <input name="config" type="checkbox"
							id="$ProblemManager" value="4" /><br /> <input name="config"
							type="checkbox" id="$VClassManager" value="5" /><br /> <input
							name="config" type="checkbox" id="$VClassTemplate" value="6" /><br />
							<input name="config" type="checkbox" id="$ProblemLevelManager"
							value="7" /><br /> -->
							<div class="checkbox">
								<label><input type="checkbox" name="config" value="0">#0
									該帳號是否有效？<br /> 若設定為失效，則該帳號無法登入系統，也看不到這個帳號。</label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="config" value="1">#1
									是否讓該題目管理員可自行「公開」題目？</label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="config" value="2">#2
									「競賽/隨堂練習管理員」的權限</label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="config" value="3">#3
									新增題目的權限 (InsertProblem) </label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="config" value="4">#4
									修改題目的權限</label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="config" value="5">#5
									「課程管理員」權限(可開設課程，進行“<fmt:message key="Contest.VContest" />”)</label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="config" value="6">#6
									開設「課程模版」的權限</label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="config" value="7">#7
									管理「題目分級」的權限(含開啟程式碼權限)</label>
							</div>
							<div class="checkbox">
								<label><input type="checkbox" name="config" value="8">#8
									管理「討論區」的權限</label>
							</div>
							<!-- 批次帳號不是改 user.config 就行，還要有 createby user。
								<div class="checkbox">
								<label><input type="checkbox" name="config" value="9">#9
									是否為批次帳號?</label>
							</div> -->

							<hr> * 教師權限應有：#2, #3, #4, #5<br>
						</td>
					</tr>
					<tr>
						<td>使用者角色</td>
						<td><c:forEach var="role"
								...
								<input name="role" type="radio" role="${user.role}"
									value="${role}" title="${role }" />${user.getRole(role, sessionScope.session_locale) } (${role.value })<br />
							</c:forEach></td>
					</tr>
				</table>
