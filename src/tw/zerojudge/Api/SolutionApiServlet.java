...
public class SolutionApiServlet extends HttpServlet implements IAccessFilter {
	...
	private ServerOutput[] getResource(Locale locale, ServerOutput[] serverOutputs,
			int wa_visible) {
		for (ServerOutput serverOutput : serverOutputs) {
			if (wa_visible == Problem.WA_visible_HIDE
					&& serverOutput.getJudgement() == ServerOutput.JUDGEMENT.WA) {
	...
	public void doPost_SolutionStatusInfo(HttpServletRequest request, HttpServletResponse response)
		...
		Solution solution = new SolutionService()
				.getSolutionById(request.getParameter("solutionid"));
		request.setAttribute("solution", solution);
		request.getRequestDispatcher("include/div/DivSolutionStatusInfo.jsp").forward(request,
				response);
		return;
	}
	...
	public void doPost_SubmitCode(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			if (lastsubmission != null && Math
					.abs(System.currentTimeMillis() - lastsubmission.getTime()) < 10 * 1000) {
			...
			if (newsolution.getContestid() > 0) { 
				...
				response.getWriter()
						.print(mapper.writeValueAsString(
								new Redirect(request.getContextPath() + SubmissionsServlet.class
	...
	public void doPost_getImgCodelocker(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		Solution solution = new SolutionService()
				.getSolutionById(request.getParameter("solutionid"));
	...
	public void doPost_getCode(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		Solution solution = new SolutionService()
				.getSolutionById(request.getParameter("solutionid"));
		OnlineUser onlineUser = new SessionScope(request).getOnlineUser();
	...
	public void doPost_getDetails(HttpServletRequest request, HttpServletResponse response)
		...
		ResourceBundle resource = ResourceBundle.getBundle("resource",
				new SessionScope(session).getSession_locale());
		Solution solution = new SolutionService()
				.getSolutionById(request.getParameter("solutionid"));
		Problemid problemid = new Problemid(request.getParameter("problemid"));
		Problem problem = new ProblemService().getProblemByProblemid(problemid);
	...
	public void doPost_getServerOutputBeans(HttpServletRequest request,
	...
	public void doPost_setServerOutput(HttpServletRequest request, HttpServletResponse response) {
		...
		ServerOutput.JUDGEMENT verdict = ServerOutput.JUDGEMENT
				.valueOf(request.getParameter("manualjudge_verdict"));
	...
	public void doPost_getSummary(HttpServletRequest request, HttpServletResponse response)
			throws IOException {
		Solution solution = new SolutionService()
				.getSolutionById(request.getParameter("solutionid"));
		OnlineUser onlineUser = new SessionScope(request).getOnlineUser();
	...
	public void doPost_removeSolution(HttpServletRequest request, HttpServletResponse response) {
		...
		new SolutionDAO().rebuiltStatistic(JudgeObject.PRIORITY.MANUALJUDGE, solution,
				solution.getJudgement());
	}
