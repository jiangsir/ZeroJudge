...
public class SolutionApiServlet extends HttpServlet implements IAccessFilter {
	...
	/**
	 * 是否請使用者「稍候再送出」for Submit Code
	 * 
	 * @return
	 */
	private int delaySubmitCodeBySecs(HttpSession session) {
		...
		long maxwait = 5 * 60 * 1000; 
		long wait = (30 + queuesize / 10 * 5) * 1000; 
		if (lastsubmission == null)
			return 0;
		else {
			long cha = Math.abs(new Date().getTime() - lastsubmission.getTime());
			if (cha > Math.min(wait, maxwait)) {
				return 0;
			} else
				return (int) ((Math.min(maxwait, wait) - cha) / 1000);
		}
	}
	...
	public void doPost_SubmitCode(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			int delayInSecs = this.delaySubmitCodeBySecs(session);
			Logger.getAnonymousLogger().info(onlineUser + " 的 SubmitCode delayInSecs=" + delayInSecs + "s。");
			if (delayInSecs > 0) {
				int queuesize = ApplicationScope.getAppConfig().getJudgeQueueSize();
				Logger.getAnonymousLogger()
						.info(onlineUser + " 的 SubmitCode 請稍後 " + delayInSecs + "s 再送出(queuesize=" + queuesize + ")。");
				if (queuesize > 50) {
					throw new JQueryException("目前待評分程式眾多，請稍候(" + delayInSecs + "s)再送出!");
				} else {
					throw new JQueryException("請稍候(" + delayInSecs + "s)再送出!");
				}
			}
			...
			if (onlineUser.isIsInContest() && !contest.getIsRunning()) {
				throw new JQueryException(contest.getBundle_Contest(session) + "已結束，無法再送出囉!");
			}
			...
			if (newsolution.getContestid() > 0) { 
				...
				response.getWriter()
						.print(mapper.writeValueAsString(new Redirect(request.getContextPath()
								+ ContestSubmissionsServlet.class.getAnnotation(WebServlet.class).urlPatterns()[0]
	...
	public void doPost_getDetails(HttpServletRequest request, HttpServletResponse response)
		...
		if (solution.getId() != 0 && problemid.getIsEmpty()) { 
			...
			new UpdateProblemServlet().AccessFilter(UserFactory.getOnlineUser(session), problemid);
	...
	public void doPost_removeSolution(HttpServletRequest request, HttpServletResponse response) {
		...
		TreeSet<Integer> solutionids = new ContestService().getSolutionidsByContest(contest, 0);
		solutionids = new ContestService().getSolutionidsByContest(contest, 0);
	...
	/**
	 * 取得 problem level 編輯紀錄<br>
	 * 
	 * @throws IOException
	 * @throws ServletException
	 */
	public void doPost_getCodeDiffs(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		OnlineUser onlineUser = new SessionScope(request).getOnlineUser();
		String solutionid = request.getParameter("solutionid");
		Solution solution = new SolutionService().getSolutionById(solutionid);

		if (!solution.getIsVisible_CodeDiff(onlineUser)) {
			throw new DataException("權限不足。");
		}
		if (!solution.isInContest()) {
			throw new DataException("程式碼比較僅支援「競賽」或「隨堂測驗」內的程式碼。");
		}
		request.setAttribute("solution_0", solution); 
		request.setAttribute("codediffs", new SolutionDAO().getCodeDiffsByContestid(solution.getId()));
		request.getRequestDispatcher("include/html/tbody_CodeDiffs.jsp").forward(request, response);
	}
