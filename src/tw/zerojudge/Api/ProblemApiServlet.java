...
//					}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		case getProblemTitles:
			...
			request.getRequestDispatcher("include/div/ProblemTitles.jsp").forward(request,
					response);
			return;
		case downloadTestdatasToZip:
			new UpdateProblemServlet().AccessFilter(request);
		...
		case getTestdataFiles:
			...
			request.getRequestDispatcher("include/div/TestdataFiles.jsp").forward(request,
					response);
			return;
		...
		case rsyncTestdatasByProblem:
			...
			try {
				new ProblemService().rsyncTestdataByProblem(problem,
						UserFactory.getOnlineUser(request));
...
//				
		...
		case getJsondetails:
			...
			try {
				response.getWriter()
						.print(mapper.writeValueAsString(problem.getServerOutputBeans()));
	...
	private void doGet_getSpecialJudgeFiles(HttpServletRequest request,
		...
		request.getRequestDispatcher("include/div/SpecialJudgeFiles.jsp").forward(request,
				response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case writeScoresTimelimits:
			...
			try {
				...
				problem.setTestfilelength(
						Math.min(problem.getScores().length, problem.getTimelimits().length));
				new ProblemService().update(problem);
			} catch (Exception e) {
				e.printStackTrace();
				throw new JQueryException(e.getLocalizedMessage());
		...
		case DetailDialog:
			request.setAttribute("serveroutputs", problem.getServeroutputs());
			request.getRequestDispatcher("include/dialog/ShowDetail.jsp").forward(request,
					response);
			return;
	 ...
	 */
	private void reJudgeProblemid(HttpServletRequest request, HttpServletResponse response,
			Problem problem) throws ServletException, IOException {
		HttpSession session = request.getSession(false);
		OnlineUser onlineUser = UserFactory.getOnlineUser(session);
		for (Solution solution : new SolutionService()
				.getSolutionsByProblemid(problem.getProblemid())) {
