...
//				break;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		switch (GETACTION.valueOf(action)) {
		case getProblemlevels:
			doGet_getProblemlevels(request, response);
			return;
		case downloadTestdatasToZip:
			new UpdateProblemServlet().AccessFilter(request);
		...
		case getProblemSettings:
			...
			break;
		case getSubmitnum:
			response.getWriter().print(new SolutionDAO().getCountByProblem(problem));
			break;
		case getVerify:
			new VerifyingProblemsServlet().AccessFilter(request);
		...
		case openProblem:
			...
			break;
//		case setDifficulty:
//			if (!problem.isShown_setDifficulty(onlineUser)) {
//				throw new JQueryException("您(" + onlineUser + ")沒有修改題目分級的權限。");
//			}
...
//			break;
	 ...
	 */
	public void doGet_getProblemlevels(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		OnlineUser onlineUser = new SessionScope(request).getOnlineUser();
		if (!onlineUser.getIsHigherEqualThanMANAGER() && !onlineUser.isCheckedConfig(User.CONFIG.ProblemLevelManager)) {
			throw new DataException("權限不足。");
		}

		int pid = Integer.parseInt(request.getParameter("pid"));
		request.setAttribute("problemlevels", new ProblemlevelDAO().getProblemlevelsByPid(pid));
		request.getRequestDispatcher("include/html/ShowProblemlevels.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		case setDifficulty:
			...
			new ProblemService().update(problem);
			return;
		case setProblemlevel:
			if (!problem.isShown_setProblemlevel(onlineUser)) {
				throw new JQueryException("您(" + onlineUser + ")沒有修改題目分級的權限。");
			}
			Problemlevel problemlevel = new Problemlevel();
			problemlevel.setLevel(request.getParameter("problemlevel"));
			problemlevel.setPid(request.getParameter("pid"));
			problemlevel.setUserid(onlineUser.getId());
			new ProblemlevelDAO().insert(problemlevel);
			return;
		...
		case getServerOutputs:
			...
			request.setAttribute("problem", problem);
			request.getRequestDispatcher("include/html/ServerOutputs.jsp").forward(request, response);
			return;
		case DetailDialog:
			request.setAttribute("serveroutputs", problem.getServeroutputs());
		...
		case doRejudge:
			...
			break;
		case doRejudgeByJudgements:
			new UpdateProblemServlet().AccessFilter(request);

			this.reJudgeProblemidByJudgements(request, response, problem);
			break;
		default:
			break;
		}
	 ...
	 * @throws ServletException
	 */
	private void reJudgeProblemidByJudgements(HttpServletRequest request, HttpServletResponse response, Problem problem)
			throws ServletException, IOException {
		...
		if (judgementsString == null) {
			solutions = new SolutionDAO().getSolutionsByProblem(problem);
		} else {
			for (String judgement : judgementsString.split(",")) {
				judgements.add(ServerOutput.JUDGEMENT.valueOf(judgement.trim()));
			}
			solutions = new SolutionDAO().getSolutionsByProblemJudgements(problem, judgements);
		}
