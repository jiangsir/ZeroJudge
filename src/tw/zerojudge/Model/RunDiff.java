...
public class RunDiff implements Runnable {
	...
	public void run() {
		...
		try {
			FileTools.writeStringToFile(new File(ApplicationScope.getSystemTmp(), "code_base"),
		...
		for (int i = 0; i < this.comparecodes.size(); i++) {
			...
			try {
				FileTools.writeStringToFile(new File(ApplicationScope.getSystemTmp(), "code_compare"),
