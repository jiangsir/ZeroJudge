...
public class SolutionJsonServlet extends HttpServlet {
	...
	private ServerOutput[] deepcopy(ServerOutput[] serverOutputs) {
		Solution temp = new Solution();
		try {
			temp.setServeroutputs(mapper.writeValueAsString(serverOutputs));
	...
	public String getJSON_ServerOutputs(HttpServletRequest request) {
		...
		try {
			...
			ServerOutput[] serverOutputs = deepcopy(solution.getServeroutputs());
			for (ServerOutput serverOutput : serverOutputs) {
				Logger.getAnonymousLogger().info("原始 serverOutput: " + mapper.writeValueAsString(serverOutput));
				...
				if (!onlineUser.getIsDEBUGGER()) {
					serverOutput.setDebug(""); 
				}
				if (solution.getLanguage().toString().equals(new Language("PYTHON").toString())
						&& serverOutput.getJudgement() == ServerOutput.JUDGEMENT.RE) {
					if (solution.getContestid() > 0
							|| solution.getProblem().getWa_visible().intValue() == Problem.WA_visible_HIDE) {
						...
						serverOutput.setHint(newhint);
						if (onlineUser.getIsHigherEqualThanMANAGER() || contest.getIsOwner(onlineUser)) {
							serverOutput.setDebug(debughint); 
						}
					}
				}

				if (serverOutput.getJudgement() == ServerOutput.JUDGEMENT.WA) {
					if (solution.getProblem().getWa_visible().intValue() == Problem.WA_visible_HIDE) {
						serverOutput.setHint("本題目的測資設定為「不公開」");
					}
					if (solution.getContestid() > 0) {
						Contest contest = new ContestService().getContestById(solution.getContestid());
						if (!contest.isVContest()) { 
							String s = contest.getBundle_Contest(onlineUser.getSession_locale()) + " 中的測資一律為「不公開」";
							serverOutput.setHint(s);
							if (contest.getIsOwner(onlineUser)) {
								serverOutput.setDebug(hint); 
							}
						} else if (!contest.isCheckedConfig_ShowWA()) { 
							String s = "本「" + contest.getBundle_Contest(onlineUser.getSession_locale())
									+ "」設定不顯示 WA 正確答案！";
							serverOutput.setHint(s);
							if (contest.getIsOwner(onlineUser)) {
								serverOutput.setDebug(hint); 
