...
public class UserStatisticServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (user != null && !user.isNullUser() && user.getRole().isLowerEqualThan(User.ROLE.MANAGER)
			...
			request.setAttribute("userProblemlevelsCount", this.getUserProblemlevelsCount(user));
			request.setAttribute("allProblemlevelCounts", new ProblemlevelDAO().getAllProblemlevelCounts());
	...
	/**
	 * 取得使用者 ac 題目分級 levels
	 * 
	 * @param user
	 * @return
	 */
	private HashMap<Integer, Integer> getUserProblemlevelsCount(User user) {
		...
		for (Problemid problemid : user.getAclist()) {
			...
			if (problem.getDisplay() == Problem.DISPLAY.open) {
				int level = problem.getProblemlevel().getLevel();
				int value = ans.get(level);
				ans.put(level, value + 1);
			}
		}
		return ans;
	}
