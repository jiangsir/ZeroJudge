...
public class UpdateProblemServlet extends HttpServlet {
	...
	public void AccessFilter(HttpServletRequest request) throws AccessException {
		...
		logger.info("沒有權限管理這個題目: request.problemid=" + request.getParameter("problemid") + ", Problemid=" + problemid);
		this.AccessFilter(onlineUser, problemid);
	}

	public void AccessFilter(OnlineUser onlineUser, Problemid problemid) throws AccessException {
		if (onlineUser == null || onlineUser.isNullOnlineUser()) {
			throw new AccessException("您沒有登入！");
		}
		logger.info("沒有權限管理這個題目: problemid=" + problemid);
		...
		if (!onlineUser.getIsMANAGER()) {
			if (!problem.getIsOwner(onlineUser)) {
				throw new AccessException(
						"您(" + onlineUser.getAccount() + ")沒有權限管理這個題目(" + problem.getProblemid() + ")！");
			}
			...
			if (((Problem.DISPLAY.open == problem.getDisplay()) || Problem.DISPLAY.verifying == problem.getDisplay()
				...
				throw new AccessException("您(" + onlineUser.getAccount() + ")沒有權限管理這個題目(" + problem.getProblemid() + "."
						+ problem.getDisplay() + ")！");
			}
		}
	}
