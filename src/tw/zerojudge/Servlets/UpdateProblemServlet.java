...
public class UpdateProblemServlet extends HttpServlet {
	...
	public void AccessFilter(OnlineUser onlineUser, Problem problem) throws AccessException {
		...
		if (!onlineUser.getIsDEBUGGER() && !onlineUser.getIsMANAGER()) {
			if (((Problem.DISPLAY.open == problem.getDisplay())
					...
					|| Problem.DISPLAY.practice == problem.getDisplay()
					|| (!"".equals(onlineUser.getAccount()))) && problem.getIsOwner(onlineUser)) {
				return;
			} else {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.setAttribute("nextaction",
				UpdateProblemServlet.class.getAnnotation(WebServlet.class).urlPatterns()[0]
						.substring(1));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		response.sendRedirect("." + this.getClass().getAnnotation(WebServlet.class).urlPatterns()[0]
				+ "?problemid=" + problemid);
	}
}
