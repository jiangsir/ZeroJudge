...
public class EditProblemsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (Problem.TAB_MYPROBLEM.equals(tabid)) {
			...
			problems = new ProblemService().getMyProblems(onlineUser, page);
		} else if (Problem.TAB_NOTOPEN.equals(tabid)) {
			tab = "tab" + new DecimalFormat("00").format(11);
			problems = new ProblemService().getNotopenProblems(onlineUser, page);
		} else if (Problem.TAB_VERIFYING.equals(tabid)) {
			tab = "tab" + new DecimalFormat("00").format(13);
			problems = new ProblemService().getVerifyingProblems(onlineUser, page);
		} else if (Problem.TAB_SPECIAL.equals(tabid)) {
			tab = "tab" + new DecimalFormat("00").format(14);
			problems = new ProblemService().getSpecialProblems(onlineUser, page);
		} else {
			problems = new ProblemService().getEditableProblems(onlineUser, rules, orderby, page);
		}
		...
		String qs = StringTool.querystingMerge(request.getQueryString());
