...
public class TestjudgeServlet extends HttpServlet implements IAccessFilter {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		int delayInSecs = this.delayTestjudgeBySecs(session);
		if (delayInSecs > 0) {
			...
			Logger.getAnonymousLogger()
					.info(onlineUser + " 的 Testjudge 請稍後 " + delayInSecs + "s 再送出(queuesize=" + queuesize + ")。");
			if (queuesize > 50) {
...
//	}
	...
	/**
	 * Testjudge 使用頻率限制。並顯示還要再等多久才能送出 Testjudge
	 * 
	 * @return
	 */
	private int delayTestjudgeBySecs(HttpSession session) {
		Date lastsubmission = new SessionScope(session).getLastsubmission();
		int queuesize = ApplicationScope.getAppConfig().getJudgeQueueSize();
		long maxwait = 100 * 1000; 
		long wait = (10 + queuesize / 10 * 5) * 1000; 
		if (lastsubmission == null)
			return 0;
		else {
	...
	private Problem makeTestProblem(int solutionid, String language, String[] testjudge_indatas,
			String[] testjudge_outdatas, Problemid problemid) {
		...
		Problem orig_problem = new ProblemService().getProblemByProblemid(problemid);
		testproblem.setJudgemode(orig_problem.getJudgemode());
		testproblem.setSpecialjudge_language(orig_problem.getSpecialjudge_language());
		testproblem.setSpecialjudge_code(orig_problem.getSpecialjudge_code());
		testproblem.setProblemid(problemid);
		return testproblem;
	}
