...
public class TestjudgeServlet extends HttpServlet implements IAccessFilter {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (lastsubmission != null
				&& Math.abs(new Date().getTime() - lastsubmission.getTime()) < 10 * 1000) {
		...
		try {
			serverOutputs = this.doTestjudge(onlineUser, contestid, problemid, testcode,
					testjudge_indata, testjudge_outdata, language, iplist);
	...
	private ServerOutput[] doTestjudge(OnlineUser onlineUser, int contestid, Problemid problemid,
			String code, String testjudge_indata, String testjudge_outdata, String language,
		...
		testsolution = this.makeTestSolution(onlineUser, code, language, iplist, contestid,
				problemid);
		testproblem = this.makeTestProblem(testsolution.getId(), language, testjudge_indata,
				testjudge_outdata);
		ServerOutput[] serverOutputs;
		ServerOutput serverOutput = new ServerOutput();
		JudgeObject judgeObject;
		try {
			judgeObject = new JudgeObject(JudgeObject.PRIORITY.Testjudge, testsolution,
					testproblem);
		} catch (Exception e) {
			serverOutputs = new ServerOutput[1];
			...
			new JudgeServer().summaryServerOutput(JudgeObject.PRIORITY.Testjudge, testproblem,
					testsolution, serverOutputs);
			e.printStackTrace();
	...
	private Solution makeTestSolution(OnlineUser onlineUser, String code, String language,
			List<IpAddress> iplist, int contestid, Problemid problemid) {
