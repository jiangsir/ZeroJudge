...
//					&& !ApplicationScope.getAppConfig().getIsCLASS_MODE()) {
//				throw new AccessException("您已參與比賽當中(<a href=\"ShowContest?contestid=" + contest.getId() + "\">"
//						+ contest.getTitle() + "</a>)，點擊進入交卷後才能解其它題目。");
//			}
			if (contest.getIsStarting() && contest.isContestProblem(problem.getProblemid())) {
				throw new AccessException("您所參加的競賽「尚未開始」，無法顯示本題目。");
			}
			if (contest.getIsPausing() && contest.isContestProblem(problem.getProblemid())) {
				throw new AccessException("您所參加的競賽「暫停中」，無法顯示本題目。");
			}
			if (contest.getIsStopped() && contest.isContestProblem(problem.getProblemid())) {
				throw new AccessException("您所參加的競賽「已結束」，無法顯示本題目。");
			}
			if (!contest.getIsRunning() && contest.isContestProblem(problem.getProblemid())) {
				throw new AccessException("您所參加的競賽的狀態並非「進行中」。因此無法觀看題目！");
			}
		} else {
			if (problem.getId() == new Problem().getId()) {
				throw new AccessException("本題目(" + problem.getProblemid() + ") 不存在！");
			} else if (Problem.DISPLAY.practice == problem.getDisplay()
					&& (onlineUser.isContestManager() || onlineUser.isVClassManager())) {
				return;
...
//						throw new AccessException("本題目(" + problem.getProblemid() + ") 正作為"
//								+ contest.getBundle_Contest(onlineUser.getSession_locale()) + "用，暫停作答。");
//					}
//				}
			}
		}
