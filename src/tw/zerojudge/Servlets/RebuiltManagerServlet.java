...
public class RebuiltManagerServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("RebuiltManager.jsp").forward(request, response);
		return;
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if ("bysolutionid".equals(rejudgeby)) {
			int solutionidfrom = Solution.parseSolutionid(request.getParameter("solutionidfrom"));
			int solutionidto = Solution.parseSolutionid(request.getParameter("solutionidto"));
			if (solutionidfrom > solutionidto) {
				int tmp = solutionidfrom;
				solutionidfrom = solutionidto;
				solutionidto = tmp;
			}
			solutions = new SolutionService().getSolutionsForRejudge(solutionidfrom, solutionidto, statusarray,
					languagearray);
			...
			for (String problemid : plist) {
				solutions.addAll(new SolutionService().getSolutionsByProblemid(new Problemid(problemid)));
			}
		} else if ("bycontestid".equals(rejudgeby)) {
			Contest contest = new ContestService().getContestById(request
					.getParameter("contestid"));
			solutions = contest.getSolutions(0);
		}
