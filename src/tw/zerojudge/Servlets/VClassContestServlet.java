	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getQueryString() == null) {
			if (onlineUser.getIsDEBUGGER()) {
				request.setAttribute("contests", new ContestService().getContestsByVclassid(0, "id DESC", page));
			} else {
				request.setAttribute("contests", new ContestService().getContestsByOwnerid(onlineUser.getId(), page));
			}
