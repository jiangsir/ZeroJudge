...
public class UpdateContestServlet extends HttpServlet implements IAccessFilter {
	...
	public void AccessFilter(OnlineUser onlineUser, Contest contest) throws AccessException {
		...
		if (!onlineUser.isContestManager()) {
			throw new AccessException("您(" + onlineUser + ") 沒有「競賽管理員」權限，無法管理這個"
					+ contest.getBundle_Contest(onlineUser.getSession_locale()) + "！");
		}
		if (!contest.getIsOwner(onlineUser)) {
			throw new AccessException(
					"您(" + onlineUser + ") 不是「" + contest.getBundle_Contest(onlineUser.getSession_locale())
							+ "擁有者」，無法管理這個" + contest.getBundle_Contest(onlineUser.getSession_locale()) + "！");
		}
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (contest.getIsaVclasstemplate()) {
			throw new AccessException(
					"本" + contest.getBundle_Contest(onlineUser.getSession_locale()) + "來自「課程模板」，無法進行編輯！");
		}
