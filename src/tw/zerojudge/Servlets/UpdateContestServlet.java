...
public class UpdateContestServlet extends HttpServlet implements IAccessFilter {
	...
	public void AccessFilter(OnlineUser onlineUser, Contest contest) throws AccessException {
		...
		if (!onlineUser.isContestManager()) {
			throw new AccessException("您(" + onlineUser + ") 沒有「競賽管理員」權限，無法管理這個競賽！");
		}
		if (!contest.getIsOwner(onlineUser)) {
			throw new AccessException("您(" + onlineUser + ") 不是「競賽擁有者」，無法管理這個競賽！");
		}
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (onlineUser.getIsDEBUGGER()) {
				...
				appConfig.setRankingmode_HSIC(
						request.getParameter(AppConfig.FIELD.rankingmode_HSIC.name()));
				appConfig.setRankingmode_NPSC(
						request.getParameter(AppConfig.FIELD.rankingmode_NPSC.name()));
				appConfig.setRankingmode_CSAPC(
						request.getParameter(AppConfig.FIELD.rankingmode_CSAPC.name()));
			...
			if (startradio != null) { 
				...
				case RUNNING:
					...
					contest.setTimelimit(request.getParameter("hours"),
							request.getParameter("mins"));

					new ContestService().update(contest);
					Task task = new TaskDAO().getTaskById(contest.getTaskid());
					new TaskService(task).doStop();

					ContestStarter contestStarter = new ContestStarter(contest,
							contest.getStarttime(), 0);
			...
			contestantsDao.rebuiltContestants(contest.getId());
