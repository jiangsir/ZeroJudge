...
public class DownloadServlet extends HttpServlet implements IAccessFilter {
	...
	private void downloadContestCSV(Integer contestid) throws UnsupportedEncodingException {
		...
		String fileName = (contest.isVContest() ? contest.getVClass().getVclassname() + "--" : "") + contest.getTitle()
				+ ".csv";
		fileName = new String(fileName.getBytes("UTF-8"), "ISO8859_1");
		ArrayList<Contestant> contestants = new ContestService().getContestantsForRanking(contestid);
		StringBuffer csvcontent = new StringBuffer(1000000);
		...
		for (int i = 0; i < contest.getProblems().size(); i++) {
			Problem problem = contest.getProblems().get(i);
			firstline.add(
					(i + 1) + ". (" + contest.getScores()[i] + "分)" + problem.getProblemid() + ": "
							+ problem.getTitle());
		}
		firstline.add("AC數");
		firstline.add("送出");
		firstline.add("耗時");
		firstline.add("分數");
		csvcontent.append(String.join(",", firstline) + "\n");

		VClassStudentDAO studentDao = new VClassStudentDAO();
		for (Contestant contestant : contestants) {
			List<String> csvline = new ArrayList<>();
			csvline.add(String.valueOf(contestant.getCurr_rank()));
			csvline.add(contestant.getUser().getAccount());
			csvline.add(contestant.getTeamname());
			csvline.add(contestant.getUser().getTruename());
			if (contest.isVContest()) {
				VClassStudent student = studentDao.getStudent(contest.getVclassid(), contestant.getUserid());
				csvline.add(student == null ? "NotStudent" : student.getComment());
			} else {
				csvline.add(contestant.getUser().getComment());
			}
			csvline.add(contestant.getSchool());
			for (int i = 0; i < contest.getProblems().size(); i++) {
				if (i >= contestant.getAclist().length) {
					csvline.add("-");
				} else {
					if (contestant.getAclist()[i].equals("AC")) {
						csvline.add("100");
					} else {
						csvline.add(contestant.getAclist()[i]);
					}
				}
			}
			csvline.add(String.valueOf(contestant.getAc()));
			csvline.add(String.valueOf(contestant.getSubmits()));
			csvline.add(String.valueOf(contestant.getPenalty()));
			csvline.add(String.valueOf(contestant.getScore()));

			csvcontent.append(String.join(",", csvline) + "\n");
		}
	 ...
	 * @throws IOException
	 */
	private void doGet_ExportContest(Integer contestid) throws IOException {
	 ...
	 * @throws IOException
	 */
	private void doGet_ImportContest(Integer contestid) throws IOException {
		HttpSession session = request.getSession(false);
	 ...
	 * 
	 * you want the browser to receive a different name than the name the file has
	 * been saved in your local database, since your local names need to be unique.
	 * 
	 * @param request
	 *                          The request
	 * @param response
