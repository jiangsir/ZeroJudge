...
public class ImportContestServlet extends HttpServlet implements IAccessFilter {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			int count = 0;
			String plainMessage = "";
			String config_newproblem = request.getParameter("importContest_config1");
			String config_newrecord = request.getParameter("importContest_config2");

			for (Part part : request.getParts()) {
				if ("importContest_JsonFile".equals(part.getName())) {
					...
					count++;
					new ContestService().ImportContestByJson(onlineUser, tmpfile, targetVclassid, config_newproblem,
							config_newrecord);
