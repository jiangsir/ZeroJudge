	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!contest.getIsOwner(onlineUser) && (ApplicationScope.getAppConfig().getIsCLASS_MODE()
			...
			request.setAttribute("solutions",
					new SolutionService().getSolutionsByUseridContestid(onlineUser.getId(), contest.getId(), 0));
		} else {
			if (contest.getId() > 0 && userid != null && userid.matches("[0-9]+")) {
				...
				request.setAttribute("solutions", new SolutionService().getSolutionsByContestidProblemid(
						contest.getId(), new ProblemService().getProblemByProblemid(new Problemid(problemid)).getId()));
