...
public class ContestSubmissionsServlet extends HttpServlet implements IAccessFilter {
	...
	public void AccessFilter(OnlineUser onlineUser, Contest contest) throws AccessException {
		...
		if (onlineUser.getJoinedcontestid().intValue() == contest.getId().intValue()
				&& contest.getIsRunning()) {
			return;
		}
		...
		if (contest.getIsRunning()
				&& onlineUser.getJoinedcontestid().intValue() != contest.getId().intValue()) {
			throw new AccessException("這個競賽(\"" + contest.getTitle() + "\")進行中，但您("
					+ onlineUser.getAccount() + ") 並未參加這個競賽");
		}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (contest.getId() > 0 && userid != null && userid.matches("[0-9]+")) {
			...
			request.setAttribute("solutions",
					new SolutionService().getSolutionsByContestidProblemid(contest.getId(),
							new ProblemService().getProblemByProblemid(new Problemid(problemid))
									.getId()));
