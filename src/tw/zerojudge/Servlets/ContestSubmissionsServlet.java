...
public class ContestSubmissionsServlet extends HttpServlet implements IAccessFilter {
	...
	public void AccessFilter(OnlineUser onlineUser, Contest contest) throws AccessException {
		...
		if (contest.getIsRunning() && onlineUser.getJoinedcontestid().intValue() != contest.getId().intValue()) {
			throw new AccessException("這個" + contest.getBundle_Contest(onlineUser.getSession_locale()) + "(\""
					+ contest.getTitle() + "\")進行中，但您(" + onlineUser.getAccount() + ") 並未參加這個"
					+ contest.getBundle_Contest(onlineUser.getSession_locale()) + "");
		}
		if (contest.getIsPausing()) {
			throw new AccessException("這個" + contest.getBundle_Contest(onlineUser.getSession_locale()) + "(\""
					+ contest.getTitle() + "\")暫停中，暫不顯示");
		}
		...
		if (contest.getIsStopped() && contest.getVisible() == Contest.VISIBLE.open
			...
			throw new AccessException(
					"這個" + contest.getBundle_Contest(onlineUser.getSession_locale()) + "(\"" + contest.getTitle()
							+ "\")這個" + contest.getBundle_Contest(onlineUser.getSession_locale()) + "並不開放參觀");
		}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (!contest.getIsOwner(onlineUser) && (ApplicationScope.getAppConfig().getIsCLASS_MODE()
				|| contest.isCheckedConfig_ShowOnlyOneContestant())) {
			int allrows = new SolutionService().getCountSolutionsByUseridContestid(onlineUser.getId(),
					contest.getId());
			request.setAttribute("allrows", allrows);
			request.setAttribute("lastpage", allrows / ApplicationScope.getAppConfig().getPageSize() + 1);

			ArrayList<Solution> solutions = new SolutionService().getSolutionsByUseridContestid(onlineUser.getId(),
					contest.getId(), page);
			request.setAttribute("solutions", solutions);
		} else {
			if (contest.getId() > 0 && userid != null && userid.matches("[0-9]+")) {
				ArrayList<Solution> solutions = new SolutionService()
						.getSolutionsByUseridContestid(Integer.parseInt(userid), contest.getId(), 0);
				request.setAttribute("allrows", solutions.size());
				request.setAttribute("lastpage", solutions.size() / ApplicationScope.getAppConfig().getPageSize() + 1);
				request.setAttribute("solutions", solutions);
			} else if (contest.getId() > 0 && problemid != null) {
				ArrayList<Solution> solutions = new SolutionService().getSolutionsByContestidProblemid(contest.getId(),
						new ProblemService().getProblemByProblemid(new Problemid(problemid)).getId());
				request.setAttribute("allrows", solutions.size());
				request.setAttribute("lastpage", solutions.size() / ApplicationScope.getAppConfig().getPageSize() + 1);
				request.setAttribute("solutions", solutions);
			} else if (contest.getId() > 0) {
				ArrayList<Solution> solutions = contest.getSolutions(page);
				ArrayList<Solution> allsolutions = contest.getSolutions(0);
				request.setAttribute("allrows", allsolutions.size());
				request.setAttribute("lastpage",
						allsolutions.size() / ApplicationScope.getAppConfig().getPageSize() + 1);
				request.setAttribute("solutions", solutions);
			}
