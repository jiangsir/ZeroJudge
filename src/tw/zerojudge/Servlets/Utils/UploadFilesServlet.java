...
public class UploadFilesServlet extends HttpServlet {
	...
	private void doPost_uploadTestdataPairs(HttpServletRequest request, HttpServletResponse response)
		...
		int max_files = Math.max(1, appConfig.getMaxTestfiles());
		if (matchedLocalfilePairs.size() > max_files) {
			StringBuffer sb = new StringBuffer();
			sb.append("您選取的測資數量超過上限 (max=" + max_files + ")。!!! \n\n");
			throw new JQueryException(sb.toString());
		} else if (notmatchedLocalfileSet.size() > 0) {
			StringBuffer sb = new StringBuffer();
			sb.append("測資上傳失敗!!! \n\n");
