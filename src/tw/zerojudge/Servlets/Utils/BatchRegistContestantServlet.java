	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		for (int i = 0; i < lines.length; i++) {
			if (!lines[i].startsWith("#")) {
				...
				newcontestant.setTeamaccount(datas[2].trim());
				newcontestant.setTeamname(datas[3].trim());
				newcontestant.setPassword(datas[4].trim());
				newcontestant.setEmail(datas[5].trim());
				newcontestant.setSchool(datas[6].trim());
