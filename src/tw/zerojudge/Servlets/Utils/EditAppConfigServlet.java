...
public class EditAppConfigServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			for (Field field : appConfig.getClass().getDeclaredFields()) {
				...
				Method method;
				try {
					method = appConfig.getClass()
							.getMethod("set" + field.getName().toUpperCase().substring(0, 1)
									+ field.getName().substring(1), String.class);
					method.invoke(appConfig,
							new Object[] { request.getParameter(parameter.name().toString()) });
	...
	private ArrayList<Banner> makeBanners(HttpServletRequest request) {
		...
		if (percents == null || bannerContents == null || percents.length == 0
	...
	private ArrayList<Problemtab> makeProblemtabs(HttpServletRequest request) {
		...
		if (!(tabids.length == tabnames.length && tabnames.length == tabdescripts.length)) {
			throw new JQueryException(
					"參數數目不搭配！tabids.length=" + tabids.length + ", tabnames.length="
							+ tabnames.length + ", tabdescripts.length=" + tabdescripts.length);
		}
