...
public class ShowOnlineUsersServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		for (HttpSession session : ApplicationScope.getOnlineSessions().values()) {
			...
			if (sessionScope.getOnlineUser() != null && !sessionScope.getOnlineUser().isNullOnlineUser()) {
		...
		Collections.sort(onlineSessionScopes, new Comparator<SessionScope>() {
			public int compare(SessionScope o1, SessionScope o2) {
				return o1.getSession_ipset().toString().compareTo(o2.getSession_ipset().toString());
			}
		});
		BannedTask bannedTask = new BannedTask();

		request.setAttribute("sortediplist", bannedTask.getSortedIPlist(bannedTask.builtIPTreeMap()));
