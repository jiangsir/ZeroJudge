...
//			String newpasswd = new Utils().randomPassword();
			...
			content += "<h1>[" + ApplicationScope.getAppConfig().getTitle() + "("
					+ sessionScope.getHostURI() + ")]</h1>";
			content += "您好：<br><br>" + "這封認證信為處理您忘記密碼，當您收到本「認證信函」後，" + "請直接點選下方連結重新設置您的密碼，無需回信。";
			String ChangePassword = ChangePasswordServlet.class.getAnnotation(WebServlet.class)
					.urlPatterns()[0];
			content += "<h2><a href=\"" + sessionScope.getHostURI() + "/" + ChangePassword
					+ "?token=" + token.getBase64() + "\">修改密碼</a></h2>";
			content += "</body></html>";

			String subject = "[密碼查詢驗證信] FROM " + ApplicationScope.getAppConfig().getTitle() + "("
					+ sessionScope.getHostURI() + ") ";
			Mailer mailer = new Mailer(user, subject, content,
					sessionScope.getHostURI().toString());
			mailer.GmailSender();
		} catch (MessagingException e) {
			e.printStackTrace();
			throw new JQueryException(
					"很抱歉，郵件因某些原因無法寄送。MessagingException:" + e.getLocalizedMessage());
		} catch (Exception e) {
			e.printStackTrace();
