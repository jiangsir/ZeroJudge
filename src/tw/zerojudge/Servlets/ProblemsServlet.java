...
public class ProblemsServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (difficulty != null) {
			this.doGet_problemlevels(request, response);
			return;
		}
	...
	private void doGet_problemlevels(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		AppConfig appConfig = ApplicationScope.getAppConfig();
		request.setAttribute("tabs", appConfig.getProblemtabs());
		String tab = "tab";
		request.setAttribute("tab", tab);
		int level = Integer.parseInt(request.getParameter("difficulty"));
		ArrayList<Problem> problems = new ProblemlevelDAO().getProblemsByLevel(level);
		request.setAttribute("problems", problems);
		request.setAttribute("lastpage", 1);
		request.setAttribute("pagenum", 1);
		request.getRequestDispatcher("Problems.jsp").forward(request, response);
