...
public class Solution {
	...
	/**
	 * 本 solution 是否為 contest 內的 solution
	 * 
	 * @return
	 */
	@JsonIgnore
	public boolean isInContest() {
		return this.getContestid() > 0;
	}
	...
	public String getCode_prefix() {
		...
		String BEGIN, END;
		if (getLanguage().getName().equals("PASCAL")) {
			BEGIN = "(*";
			END = "*)";
		} else if (getLanguage().getName().equals("BASIC")) {
			BEGIN = "/'";
			END = "'/";
		} else {
			BEGIN = "/*";
	...
	/**
	 * 是否有權顯示 CoddDiff button
	 * 
	 * @param onlineUser
	 * @return
	 */
	@JsonIgnore
	public boolean getIsVisible_CodeDiff(OnlineUser onlineUser) {
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}
		Contest contest = this.getContest();
		if ((onlineUser.getIsHigherEqualThanMANAGER() || contest.getIsOwner(onlineUser))
				&& this.getJudgement() == JUDGEMENT.AC) {
			return true;
		}
		return false;
	}
	...
	/**
	 * 只使用在 ImportContest，代為顯示 teamaccount
	 */
	...
	/**
	 * 只使用在 ImportContest，代為顯示 teamname
	 */
