...
public class User implements Serializable {
	...
	public static void checkAccount(String account) throws DataException {
		...
		if (account != null && account.length() >= length && account.matches("[a-zA-Z_0-9]+")) {
			...
			throw new DataException("錯誤!! (" + account + ") 不符合帳號規則",
					" 請使用文數字及_，並至少用 " + length + " 個字元");
		}
...
//	}
	...
	public HashSet<Class<? extends HttpServlet>> getDenyServlet() {
		if (this.isIsInContest()) {
			for (Class<? extends HttpServlet> servletClazz : RoleFilter.class
					.getAnnotation(Privilege.class).remove_UserInContest()) {
	...
	public HashSet<Class<? extends HttpServlet>> getAllowServlet() {
		for (Class<? extends HttpServlet> servletClazz : ApplicationScope.getRoleMap()
				.get(this.getRole())) {
...
//		}
		if (this.isCheckedConfig(CONFIG.ContestManager)) {
			for (Class<? extends HttpServlet> servletClazz : RoleFilter.class
					.getAnnotation(Privilege.class).add_ContestManager()) {
				this.allowServlet.add(servletClazz);
			}
		}
		if (this.isCheckedConfig(CONFIG.ProblemManager)) {
			for (Class<? extends HttpServlet> servletClazz : RoleFilter.class
					.getAnnotation(Privilege.class).add_ProblemManager()) {
				this.allowServlet.add(servletClazz);
			}
		}
		if (this.isCheckedConfig(CONFIG.VClassManager)) {
			for (Class<? extends HttpServlet> servletClazz : RoleFilter.class
					.getAnnotation(Privilege.class).add_VClassManager()) {
				this.allowServlet.add(servletClazz);
			}
		}
		if (this.isCheckedConfig(CONFIG.InsertProblem)) {
			for (Class<? extends HttpServlet> servletClazz : RoleFilter.class
					.getAnnotation(Privilege.class).add_InsertProblem()) {
				this.allowServlet.add(servletClazz);
			}
		}
		return allowServlet;
...
//		}
	...
	public boolean islegalEmail(String email) {
		if (email != null && email.toLowerCase().matches(
	...
	public boolean getIsAuthhost_Google() {
		return this.getAuthhost() == AUTHHOST.Google
				&& ApplicationScope.getAppConfig().getIsGoogleLoginSetup();
	}
