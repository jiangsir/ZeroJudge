...
import tw.jiangsir.Utils.Filters.Filter5_RoleFilter;
...
public class User implements Serializable {
	...
	public static final String UserRankRule = "ac DESC, id DESC";
	...
	public static void checkAccount(String account) throws DataException {
		...
		if (ApplicationScope.getAppConfig().getIsCLASS_MODE()) {
			/**
			 * CLASS_MODE 下，允許 @ 作為帳號名稱 "@schoolid"
			 */
	...
	public String getRole(Locale locale) {
		return getRole(this.getRole(), locale);
	}
	 ...
	 * @return
	 */
	public String getRole(ROLE role, Locale locale) {
		if (role == null) {
			role = this.getRole();
		}
		if (locale == null) {
		...
		return resource.getString("User.Role." + role);
	}
	...
	public User getCreatebyUser() {
		return new UserService().getUserById(this.getCreateby());
	}
	...
	public void setUsername(String username) {
		...
		this.username = StringTool.escapeHtmlTag(username.trim());
	}
	 ...
	 * @return
	 */
	public Integer enableConfig(CONFIG config) {
		this.config = this.config | (1 << config.ordinal());
		return this.config;
	}
	 ...
	 * @return
	 */
	public Integer enableConfig(int ordinal) {
		return this.config | (1 << ordinal);
	}
	 ...
	 * @return
	 */
	public Integer disableConfig(CONFIG config) {
		return this.config & (Integer.MAX_VALUE - (1 << config.ordinal()));
	}
	 ...
	 * @return
	 */
	public Integer changeConfig(CONFIG config) {
		return this.config ^ (1 << config.ordinal());
	}
	...
	public void setPasswd(String passwd, String passwd2) throws DataException {
	...
	public boolean getIsQualifiedAuthor() {
		return getIsHigherEqualThanMANAGER() || isCheckedConfig(CONFIG.QualifiedAuthor);
	}
	...
	public boolean isContestManager() {
		return getIsHigherEqualThanMANAGER() || isCheckedConfig(CONFIG.ContestManager);
	}

	public boolean isInsertProblem() {
		return getIsHigherEqualThanMANAGER() || isCheckedConfig(CONFIG.InsertProblem);
	}

	public boolean isProblemManager() {
		return getIsHigherEqualThanMANAGER() || isCheckedConfig(CONFIG.ProblemManager);
	}

	public boolean isVClassManager() {
		return getIsHigherEqualThanMANAGER() || isCheckedConfig(CONFIG.VClassManager);
	}

	public boolean isVClassTemplate() {
		return getIsHigherEqualThanMANAGER() || isCheckedConfig(CONFIG.VClassTemplate);
	}

	public boolean isProblemLevelManager() {
		return getIsHigherEqualThanMANAGER() || isCheckedConfig(CONFIG.ProblemLevelManager);
	}

	public boolean isForumManager() {
		return getIsHigherEqualThanMANAGER() || isCheckedConfig(CONFIG.ForumManager);
	}
	 ...
	 * @return
	 */
	public boolean getIsBatchedUser() {
		return isCheckedConfig(CONFIG.BatchedUser) && this.getCreateby().intValue() > 0
				&& this.getId().intValue() != this.getCreateby().intValue();
	}
	...
	public HashSet<Class<? extends HttpServlet>> getDenyServlet() {
		if (this.isIsInContest()) {
			for (Class<? extends HttpServlet> servletClazz : Filter5_RoleFilter.class.getAnnotation(Privilege.class)
