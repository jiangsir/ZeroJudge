...
import tw.zerojudge.DAOs.ProblemlevelDAO;
...
//	public static final double MAX_DATAPAIRS = 20; 
//	public static final double MIN_TIMELIMIT = 0.1; 
//	public static final double MAX_SINGLE_TIMELIMIT = 3; 
//	public static final double MAX_TOTAL_TIMELIMIT = 10; 
	...
	/*
	 * 20090505 以 hash problem 取代 public static Integer hashPid(String problemid) {
	 * if (ProblemsDAO.ProblemPids.get(problemid) == null) {
	 ...
	 * ProblemsDAO.ProblemTitles.put(problemid, new Problem(problemid) .getTitle());
	 * } return ProblemsDAO.ProblemTitles.get(problemid); }
	 * 
	...
	public String getContent() {
		this.content = HtmlUtils.sanitizeHtml(content);
		return content;
	}
	...
	public String getHint() {
		return HtmlUtils.sanitizeHtml(hint);
	}
	...
	public String[] getSampleinputs() {
		try {
			return mapper.readValue(this.getSampleinput(), String[].class);
		} catch (JsonParseException e) {
			return new String[] { this.getSampleinput() };
	...
	public String[] getSampleoutputs() {
		try {
			return mapper.readValue(this.getSampleoutput(), String[].class);
		} catch (JsonParseException e) {
			return new String[] { this.getSampleoutput() };
		} catch (JsonMappingException e) {
			return new String[] { this.getSampleoutput() };
		} catch (IOException e) {
			return new String[] { this.getSampleoutput() };
	...
	public String getTheinput() {
		return HtmlUtils.sanitizeHtml(theinput);
	}

	public String getTheoutput() {
		return HtmlUtils.sanitizeHtml(theoutput);
	}
	...
	public String getTabid() {
		if ("".equals(this.tabid)) {
			this.tabid = ApplicationScope.getAppConfig().getProblemtabs().get(0).getId();
		}
		return tabid;
	}
	...
	public void setTimelimits(double[] timelimits) throws DataException {
		//
		this.timelimits = timelimits;
	}

	/**
	 * 傳入 timelimits double[]
	 * 
	 * @param timelimits
	 * @throws DataException
	 */
	public void checkTimelimits(double[] timelimits) throws DataException {
		double totaltimelimit = 0;
		for (double timelimit : timelimits) {
			if (timelimit < ApplicationScope.getAppConfig().getMinSingleTimeLimit()) {
				throw new DataException("最低時間限制為 " + Problem.DecimalFormat_ProblemTimeLimit
						.format(ApplicationScope.getAppConfig().getMinSingleTimeLimit()) + "s !");
			}
			if (timelimit > ApplicationScope.getAppConfig().getMaxSingleTimeLimit()) {
				throw new DataException("單一測資點時間上限為 " + Problem.DecimalFormat_ProblemTimeLimit
						.format(ApplicationScope.getAppConfig().getMaxSingleTimeLimit()) + "s !");
			}
			totaltimelimit += timelimit;
		}
		if (totaltimelimit > ApplicationScope.getAppConfig().getMaxTotalTimeLimit()) {
			throw new DataException("總時限不能超過 " + Problem.DecimalFormat_ProblemTimeLimit
					.format(ApplicationScope.getAppConfig().getMaxTotalTimeLimit()) + "s.");
		}
	}

	/**
	 * 傳入 timelimits string[]
	 * 
	 * @param timelimits
	 * @throws DataException
	 */
	public void checkTimelimits(String[] stringtimelimits) throws DataException {
		if (stringtimelimits == null || stringtimelimits.length == 0) {
			throw new DataException("timelimits 參數不存在(" + timelimits + ")!!");
		}

		double[] timelimits = new double[stringtimelimits.length];
		for (int i = 0; i < stringtimelimits.length; i++) {
			try {
				timelimits[i] = Double.parseDouble(stringtimelimits[i].trim());
			} catch (NumberFormatException e) {
				e.printStackTrace();
				throw new DataException("測資時間設定必須為浮點數");
			}
		}
		this.checkTimelimits(timelimits);
	}

	/**
	 * 從資料庫 string 讀入 timelimit
	 * 
	 * @param timelimits
	 * @throws DataException
	 */
...
//	}
	...
	/**
	 * 從 request.getParameterValues() String[]
	 * 
	 * @param timelimits
	 * @throws DataException
	 */
...
//	}
	...
	public String getTESTJUDGE_FILENAME(int solutionid, int index) {
		return "Testjudge_" + solutionid + "/" + "Testjudge_" + solutionid + (index == 0 ? "" : "_" + index);
	}
	...
	public ArrayList<TestdataPair> getTestdataPairs() {
		...
		for (int index = 0; index < this.getTestfilelength(); index++) {
	...
	public boolean isShown_setDifficulty(OnlineUser onlineUser) {
		if (onlineUser == null || onlineUser.isNullOnlineUser()) {
			return false;
		}
		if (onlineUser.getIsHigherEqualThanMANAGER() || onlineUser.isProblemLevelManager()
				|| this.getIsOwner(onlineUser)) {
			return true;
		}
		return false;
	}
	...
	/**
	 * 判斷是否可以管理 problemlevel
	 * 
	 * @param onlineUser
	 * @return
	 */
	public boolean isShown_setProblemlevel(OnlineUser onlineUser) {
		if (onlineUser == null || onlineUser.isNullOnlineUser()) {
			return false;
		}
		if (onlineUser.getIsHigherEqualThanMANAGER() || onlineUser.isProblemLevelManager()) {
	...
	public Problemlevel getProblemlevel() {
		return new ProblemlevelDAO().getProblemlevelByPid(this.getId());
	}
