...
public class Problem {
		...
		for (String score : scores) {
			...
			if (Integer.parseInt(score.trim()) < Problem.MIN_SCORE) {
				throw new DataException(
						"(" + problemid + ") 各測資配分必須 >= " + Problem.MIN_SCORE + " 分");
			}
	...
	public void setSpecialjudge_language(String specialjudge_language) {
		...
		if ("".equals(specialjudge_language)) {
			...
			this.setSpecialjudge_language(
					new Language(specialjudge_language, specialjudge_language.toLowerCase()));
		}
	}
	...
	public void setMemorylimit(Integer memorylimit) throws DataException {
		if (memorylimit > Problem.MAX_MEMORYLIMIT || memorylimit < Problem.MIN_MEMORYLIMIT) {
			throw new DataException("記憶體(" + memorylimit + ")應指定在 " + Problem.MIN_MEMORYLIMIT
					+ "MB 到 " + Problem.MAX_MEMORYLIMIT + "MB 之間。");
		}
	...
	public String getSummary() {
		...
		case AC:
			...
			summary = Solution.parseTimeusage(timeusage) + ", "
					+ Solution.parseMemoryusage(memoryusage);
			break;
		case TLE:
			timeusage = 0;
			for (ServerOutput serverOutput : this.getServeroutputs()) {
	...
	public void setServeroutputs(String serveroutputs) {
		try {
			...
			logger.info("JsonParseException: " + e.getLocalizedMessage() + ": serverouts="
					+ serveroutputs);
			this.setServeroutputs(new ServerOutput[] {});
		} catch (JsonMappingException e) {
			logger.info("JsonMappingException: " + e.getLocalizedMessage() + ": serverouts="
					+ serveroutputs);
			this.setServeroutputs(new ServerOutput[] {});
		} catch (IOException e) { 
			logger.info(
					"IOException: " + e.getLocalizedMessage() + ": serverouts=" + serveroutputs);
			this.setServeroutputs(new ServerOutput[] {});
		}
	}
	...
	public void checkTimelimits(String[] timelimits) throws DataException {
		...
		if (timelimits.length == 1) {
			...
			if (timelimit < Problem.MIN_TIMELIMIT) {
				throw new DataException("最低時間限制為 "
						+ Problem.DecimalFormat_ProblemTimeLimit.format(Problem.MIN_TIMELIMIT)
						+ "秒!");
			}
			if (timelimit > Problem.MAX_SINGLE_TIMELIMIT) {
				throw new DataException("單一測資點時間上限為 " + Problem.DecimalFormat_ProblemTimeLimit
						.format(Problem.MAX_SINGLE_TIMELIMIT) + " 秒!");
			}
		} else {
			...
			if (totaltimelimit > Problem.MAX_TOTAL_TIMELIMIT) {
				throw new DataException("時限總和不能超過 "
						+ Problem.DecimalFormat_ProblemTimeLimit.format(Problem.MAX_TOTAL_TIMELIMIT)
						+ " 秒。");
			}
		}
	}
	...
	public void setTitle(String title) {
		if (title == null || "".equals(title.trim())
				|| new Problem().getTitle().equals(title.trim())) {
			return;
		}
	...
	public String getTESTJUDGE_FILENAME(int solutionid, int index) {
		return "Testjudge_" + solutionid + "/" + "Testjudge_" + solutionid
				+ (index == 0 ? "" : "_" + index);
	}
	...
	public int[] getImageInfos() {
		...
		for (Element img : imglist) {
			...
			String showImage = new ShowImageServlet().getClass().getSimpleName()
					.replaceAll("Servlet", "");
	...
	public ArrayList<TestdataPair> getTestdataPairs() {
		...
		for (int index = 0; testdataPath.exists()
				&& index < testdataPath.listFiles().length / 2; index++) {
