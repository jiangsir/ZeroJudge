...
public class OnlineUser extends User implements Serializable, HttpSessionBindingListener {
	...
	public OnlineUser(HttpSession session, User user) {
		...
		for (Field field : User.class.getDeclaredFields()) {
			...
			Method getter;
			try {
				getter = User.class.getMethod("get" + field.getName().toUpperCase().substring(0, 1)
						+ field.getName().substring(1));
				value = getter.invoke((User) user);
				if (value == null) {
					continue;
				}
				Method setter = User.class
						.getMethod(
	...
	public void doJoinContest(Contest contest) throws RoleException {
		...
		contestant.setFinishtime(new Timestamp(
				new Date(contest.getStarttime().getTime() + contest.getTimelimit()).getTime()));

		if (new ContestantDAO().getContestantByUseridContestid(this.getId(), contest.getId())
				.getIsNull()) {
			int contestantid = new ContestantDAO().insert(contestant);
	...
	public String toString() {
		return "userid=" + this.getId() + ", account=" + this.getAccount() + "(ROLE="
				+ this.getRole() + ")";
	}
	...
	public boolean getIsTestjudgeAccessible() {
		try {
			new TestjudgeServlet().AccessFilter(this,
					new ContestService().getContestById(this.getJoinedcontestid()));
			return true;
		} catch (AccessException e) {
			return false;
		}
