...
public class Contest implements Serializable {
	...
	public static class Pausepoint {
		...
		public boolean isFinished() {
			if (begintime.getTime() != 0L && endtime.getTime() != 0L
	...
	public void setTimelimit(Long timelimit) throws DataException {
		if (timelimit <= 0) {
			...
			throw new DataException("目前競賽進行時間上限定義為 " + Contest.MAX_CONTEST_DAYS + " 天("
	...
	public void setTimelimit(String hours, String mins) {
		...
		this.setTimelimit(
				Integer.parseInt(hours.trim()) * 3600000 + Integer.parseInt(mins.trim()) * 60000L);
	}
	...
	public String getPausepoints_HTML() {
		...
		for (Pausepoint pausepoint : this.getPausepoints()) {
			...
			int mins = secs / 60;
			int hours = mins / 60;
			html.append("暫停" + index++ + ": " + (hours == 0 ? "" : hours + "h")
					+ (mins == 0 ? "" : mins % 60 + "m") + secs % 60 + "s" + "@"
					+ new Utils().parseDatetime(pausepoint.getBegintime()) + "<br/>");
		}
	...
	public void setPausepoints(String pausepoints) {
		...
		try {
			this.setPausepoints((ArrayList<Pausepoint>) mapper.readValue(pausepoints,
					new TypeReference<ArrayList<Pausepoint>>() {
					}));
	...
	public void setRemovedsolutionids(String removedsolutionids) {
		TreeSet<Integer> descSet = new TreeSet<Integer>(
				new ContestFactory.DESCComparator<Integer>());
	...
	public void doRunning() {
		...
		if (this.isCheckedConfig(Contest.CONFIG.AutoRunning)) {
			ContestCloser contestCloser = new ContestCloser(this, this.getStarttime(),
					getTimelimit());
	...
	public void doRestart() throws DataException {
		if (!this.isCheckedConfig_AutoRunning()) {
			...
			if (this.getPausepoints().size() > 0) {
				Pausepoint lastPausepoint = this.getPausepoints()
						.get(this.getPausepoints().size() - 1);
				lastPausepoint.setEndtime(new Date());
			}
			new ContestService().update(this);
			return;
