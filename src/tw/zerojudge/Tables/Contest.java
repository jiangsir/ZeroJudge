...
public class Contest implements Serializable {
	...
	@Persistent(name = "vclasstemplateid")
	...
	public void setTitle(String title) {
		if (title == null) {
			...
			throw new DataException("主題不可以為空字串。");
		}
	...
	/**
	 * 檢查 problemids, 與 scores 數量是否吻合。
	 * 
	 * @return
	 */
	public boolean checkProblemidsScores() {
		if (this.getVisible() != Contest.VISIBLE.remove && this.getScores().length != this.getProblemids().size()) {
			throw new DataException(
					"題目數量(" + this.getProblemids().size() + ")與分數數量(" + this.getScores().length + ")不相同！請修正。");
		}
		return true;
	}
	...
	public boolean checkProblemids() throws DataException {
		...
		for (Problemid problemid : problemids) {
			...
			if (this.getOwnerid().intValue() != problem.getOwnerid().intValue()
				...
				throw new DataException("他人的題目(" + problemid + ")已隱藏，不能作為 " + this.getBundle_Contest() + " 題目。");
			}
	...
	public void setTimelimit(Long timelimit) throws DataException {
		if (timelimit <= 0) {
			throw new DataException(this.getBundle_Contest() + "進行時間設定有誤！設定值：" + timelimit);
		} else if (timelimit > 60000L * 60 * 24 * Contest.MAX_CONTEST_DAYS) {
			throw new DataException("目前" + this.getBundle_Contest() + "進行時間上限定義為 " + Contest.MAX_CONTEST_DAYS + " 天("
					+ Contest.MAX_CONTEST_DAYS * 24 + "小時)，請重新填寫。");
		}
	 ...
	 * 開始暫停使用 contest.dostop 應該可以完全由 doPause 取代。
	 * 
	 * @param contestid
	...
	@JsonIgnore
	public boolean isCheckedConfig_ShowOnlyOneContestant() {
		return isCheckedConfig(CONFIG.ShowOnlyOneContestant);
	}
	...
	@JsonIgnore
	public int getConfig_ShowOnlyOneContestant() {
		return CONFIG.ShowOnlyOneContestant.ordinal();
	}
	...
	public ArrayList<Solution> getSolutions(int page) {
		...
		TreeSet<Integer> solutionids = new ContestService().getSolutionidsByContest(this, page);
	...
	public Contestant getContestantByUserid(int userid) {
		Contestant contestant = new ContestantDAO().getContestantByUseridContestid(userid, this.getId());
		return contestant;
	}
	 ...
	 */
	@JsonIgnore
	public boolean getIsaVclasstemplate() {
		return this.getVclasstemplateid() != 0;
	}
	...
	public VClass getVClass() {
		if (this.isVContest()) {
			return new VClassDAO().getVClassById(this.getVclassid());
		}
		return new VClass(); 
	}
	 ...
	 */
	@JsonIgnore
	public String getBundle_Contest() {
		return this.getBundle_Contest(Locale.getDefault());
	}
	 ...
	 */
	@JsonIgnore
	public String getBundle_Contest(Locale locale) {
		if (locale == null) {
			locale = Locale.getDefault();
		}
		ResourceBundle resource = ResourceBundle.getBundle("resource", locale);
		if (this.isVContest()) {
			return resource.getString("Contest.VContest");
		} else {
			return resource.getString("Contest.Contest");
		}
	}
	 ...
	 */
	@JsonIgnore
	public String getBundle_Contest(HttpSession session) {
		return this.getBundle_Contest(new SessionScope(session).getSession_locale());
	}

	public boolean visible_rebuiltContest(HttpSession session) {
		OnlineUser onlineUser = new SessionScope(session).getOnlineUser();
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}
		try {
			new RebuiltContestServlet().AccessFilter(onlineUser, this);
			return true;
	...
	/**
	 * 顯示 Export Contest 按鈕
	 * 
	 * @return
	 */
	public boolean visible_ExportContest(HttpSession session) {
		OnlineUser onlineUser = new SessionScope(session).getOnlineUser();
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}
		return onlineUser.getIsHigherEqualThanMANAGER();
	}

	public boolean visible_UpdateContest(HttpSession session) {
		OnlineUser onlineUser = new SessionScope(session).getOnlineUser();
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}
		try {
			new UpdateContestServlet().AccessFilter(onlineUser, this);
			return true;
		} catch (AccessException e) {
			return false;
		}
	 ...
	 * @return
	 */
	public boolean visible_stopContest(OnlineUser onlineUser) {
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}

		if ((onlineUser.getIsHigherEqualThanMANAGER() || this.getIsOwner(onlineUser))
				&& (this.getIsRunning() || this.getIsPausing())) {
			return true;
		}
		return false;
	}
	 ...
	 * @return
	 */
	public boolean visible_startContest(OnlineUser onlineUser) {
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}
		if (onlineUser.getIsDEBUGGER() && this.getIsStopped()) {
			return true;
		}
		if ((onlineUser.getIsDEBUGGER() || this.getIsOwner(onlineUser))
				&& (this.getIsStarting() || this.getIsSuspending())) {
			return true;
		}
		return false;
	}
	 ...
	 * @return
	 */
	public boolean visible_pauseContest(OnlineUser onlineUser) {
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}

		if (onlineUser.getIsDEBUGGER() && this.getIsStopped() && this.getProblemids().size() != 0) {
			return true;
		}
		if ((onlineUser.getIsDEBUGGER() || this.getIsOwner(onlineUser)) && (this.getIsRunning())
				&& this.getProblemids().size() != 0) {
			return true;
		}
		return false;
	}
	 ...
	 * @return
	 */
	public boolean visible_resumeContest(OnlineUser onlineUser) {
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}
		...
		if ((onlineUser.getIsDEBUGGER() || this.getIsOwner(onlineUser)) && (this.getIsPausing())
				&& this.getProblemids().size() != 0) {
			return true;
		}
		return false;
	 ...
	 * @return
	 */
	public boolean visible_EditContestants(OnlineUser onlineUser) {
		try {
			if (onlineUser == null) {
				onlineUser = UserFactory.getNullOnlineUser();
			}
			new EditContestantsServlet().AccessFilter(onlineUser, this);
			return true;
		} catch (AccessException e) {
			return false;
		}
	...
	/**
	 * 判斷是否顯示 Contest JoinButton。
	 * 出現 JoinButton 代表尚未參加 contest, 尚未退出前一個 contest。
	 * 
	 * @param onlineUser
	 * @param contest
	 * @return
	 */
	public boolean visible_JoinContestButton(OnlineUser onlineUser) {
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}
		...
		if (onlineUser.getJoinedcontestid().intValue() == this.getId().intValue() || this.getIsStopped()) {
			return false;
		}
		return true;
	}
	...
	/**
	 * 判斷 onlineUser 是否已參加其它 contest
	 * 
	 * @param onlineUser
	 * @param contest
	 * @return
	 */
	public boolean isJoinedOtherContest(OnlineUser onlineUser) {
		if (onlineUser != null && onlineUser.getJoinedcontestid() != 0
				&& onlineUser.getJoinedcontestid().intValue() != this.getId().intValue()) {
			return true;
		}
		return false;
	}
	...
	/**
	 * 判斷 onlineUser 是否已參加 ThisContest
	 * 
	 * @param onlineUser
	 * @return
	 */
	public boolean isJoinedThisContest(OnlineUser onlineUser) {
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}
		return onlineUser.getJoinedcontestid().intValue() == this.getId().intValue();
	}
	 ...
	 * @return
	 */
	public boolean visible_ContestProblems(OnlineUser onlineUser) {
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}
		...
		if (this.getIsRunning() && this.isJoinedThisContest(onlineUser)) {
			return true;
		}
	...
	/**
	 * 在 contest.Starting 等待啟動的過程暫時離開 contest
	 * 
	 * @param onlineUser
	 * @param contest
	 * @return
	 */
	public boolean visible_LeaveContest(OnlineUser onlineUser) {
		if (onlineUser == null) {
			onlineUser = UserFactory.getNullOnlineUser();
		}

		try {
			new LeaveContestServlet().AccessFilter(onlineUser, this);
			return true;
		} catch (AccessException e) {
			return false;
		}
