...
import tw.jiangsir.Utils.Tools.HtmlUtils;
...
import tw.zerojudge.Servlets.InsertBatchedUsersServlet;
...
public class AppConfig {
	...
	private URL serverUrl = getDefaultServerUrl();
	...
	private int maxCodeLength = 10 * 1024;
	@AppConfigField(name = FIELD.maxTestdataBytes)
	private int maxTestdataBytes = 10 * 1024 * 1024; 
	...
	@AppConfigField(name = FIELD.enableReCaptcha)
	private boolean enableReCaptcha = false;
	@AppConfigField(name = FIELD.site_key)
	private String site_key = "";
	@AppConfigField(name = FIELD.secret_key)
	private String secret_key = "";

	@AppConfigField(name = FIELD.enableGoogleLogin)
	...
	@AppConfigField(name = FIELD.bitconfig)
	private Integer bitconfig = Integer.valueOf("000000010", 2);

	@AppConfigField(name = FIELD.maxTestfiles)
	private Integer maxTestfiles = 20;
	@AppConfigField(name = FIELD.minSingleTimeLimit)
	private Double minSingleTimeLimit = 0.1; 
	@AppConfigField(name = FIELD.maxSingleTimeLimit)
	private Double maxSingleTimeLimit = 3.0; 
	@AppConfigField(name = FIELD.maxTotalTimeLimit)
	private Integer maxTotalTimeLimit = 40; 
	@AppConfigField(name = FIELD.maxImportProblemsPerDay)
	private Integer maxImportProblemsPerDay = 10; 
	...
	final private LinkedHashSet<Class<? extends HttpServlet>> PRIVILEGE_VClassManager = new LinkedHashSet<Class<? extends HttpServlet>>() {
...

		{
			...
			add(InsertBatchedUsersServlet.class);
	...
	public void setEnableMailer(String enableMailer) {
		if (enableMailer == null || "".equals(enableMailer.trim())) {
			this.setEnableMailer(false);
		} else {
	...
	public boolean getIsCLASS_MODE() {
		return this.getSystemMode() == SYSTEM_MODE.CLASS_MODE;
	}

	public boolean getIs_CONTEST_MODE() {
		return this.getSystemMode() == SYSTEM_MODE.CONTEST_MODE;
	}

	public boolean isTRAINING_MODE() {
		return this.getSystemMode() == SYSTEM_MODE.TRAINING_MODE;
	}
	...
	public boolean isPROBLEMS_MODE() {
		return this.getSystemMode() == SYSTEM_MODE.PROBLEMS_MODE;
	}
	...
	public boolean isCLOSE_MODE() {
		return this.getSystemMode() == SYSTEM_MODE.CLOSE_MODE;
	}
	...
	public URL getDefaultServerUrl() {
		try {
			return new URL("http://127.0.0.1/ZeroJudge_Server/");
	...
	public void setServerUrl(String serverUrl) {
		try {
			...
			e.printStackTrace();
			this.setServerUrl(this.getDefaultServerUrl());
		}
	}
	...
	public PriorityQueue<JudgeObject> getJudgeQueue() {
		return JudgeFactory.getJudgeQueue();
	}
	...
	public int getMaxTestdataBytes() {
		return maxTestdataBytes;
	}

	public void setMaxTestdataBytes(Integer maxTestdataBytes) {
		this.maxTestdataBytes = maxTestdataBytes;
	}

	public void setMaxTestdataBytes(String maxTestdataBytes) {
		try {
			this.setMaxTestdataBytes(Integer.parseInt(maxTestdataBytes));
		} catch (Exception e) {
			this.setMaxTestdataBytes(10 * 1024 * 1024);
		}
	}
	...
	public ArrayList<Banner> getBanners() {
		for (Banner banner : banners) {
			banner.setContent(HtmlUtils.sanitizeHtml(banner.getContent()));
		}
		return banners;
	}
	...
	public String getBannerContent() {
		...
		return HtmlUtils.sanitizeHtml(bannerContent);
	}
	...
	public void setEnableReCaptcha(String enableReCaptcha) {
		if (enableReCaptcha == null || "".equals(enableReCaptcha.trim())) {
			this.setEnableReCaptcha(false);
		} else {
			this.setEnableReCaptcha(Boolean.valueOf(enableReCaptcha));
		}
	}
	...
	public void setEnableGoogleLogin(String enableGoogleLogin) {
		if (enableGoogleLogin == null || "".equals(enableGoogleLogin.trim())) {
			this.setEnableGoogleLogin(false);
		} else {
			this.setEnableGoogleLogin(Boolean.valueOf(enableGoogleLogin));
		}
	}
	 ...
	 * @return
	 */
	public void doEnableConfig(BITCONFIG bitconfig) {
		this.setBitconfig(this.getBitconfig().intValue() | (1 << bitconfig.ordinal()));
	}
	 ...
	 * @return
	 */
	public void doDisableConfig(BITCONFIG bitconfig) {
		this.setBitconfig(getBitconfig().intValue() & (Integer.MAX_VALUE - (1 << bitconfig.ordinal())));
	}
	 ...
	 * @return
	 */
	public void doExchangeConfig(BITCONFIG bitconfig) {
		this.setBitconfig(getBitconfig().intValue() ^ (1 << bitconfig.ordinal()));
	}
	 ...
	 * @return
	 */
	public void doAssignConfig(BITCONFIG bitconfig, boolean value) {
		if (value == true) {
			this.doEnableConfig(bitconfig);
		} else {
			this.doDisableConfig(bitconfig);
		}
	}
	 ...
	 */
	@JsonIgnore
	public boolean isCheckedConfig(BITCONFIG bitconfig) {
		int x = getBitconfig().intValue() & (1 << bitconfig.ordinal());
		return x > 0 ? true : false;
	}

	@JsonIgnore
	public boolean isChecked_rejudgeable() {
		return isCheckedConfig(BITCONFIG.rejudgeable);
	}

	@JsonIgnore
	public boolean isChecked_registerable() {
		return isCheckedConfig(BITCONFIG.registerable);
	}

	@JsonIgnore
	public boolean isChecked_applyTeacher() {
		return isCheckedConfig(BITCONFIG.applyTeacher);
	}

	@JsonIgnore
	public boolean isChecked_applyProblemManager() {
		return isCheckedConfig(BITCONFIG.applyProblemManager);
	}

	public Integer getBitconfig() {
		return bitconfig;
	}

	public void setBitconfig(Integer bitconfig) {
		this.bitconfig = bitconfig;
	}

	public void setBitconfig(String bitconfig) {
		try {
			this.bitconfig = Integer.parseInt(bitconfig.trim());
		} catch (Exception e) {
			this.bitconfig = Integer.valueOf("000000010", 2);
		}
	}

	public Integer getMaxTestfiles() {
		return maxTestfiles;
	}

	public void setMaxTestfiles(Integer maxTestfiles) {
		if (maxTestfiles > 100) {
			maxTestfiles = 100;
		}
		if (maxTestfiles < 1) {
		...
		this.maxTestfiles = maxTestfiles;
	}

	public void setMaxTestfiles(String maxTestfiles) {
		if (maxTestfiles == null || !maxTestfiles.matches("[0-9]+")) {
			return;
		}
		this.setMaxTestfiles(Integer.parseInt(maxTestfiles.trim()));
	}

	public Double getMinSingleTimeLimit() {
		return minSingleTimeLimit;
	}

	public void setMinSingleTimeLimit(Double minSingleTimeLimit) {
		this.minSingleTimeLimit = minSingleTimeLimit;
	}

	public void setMinSingleTimeLimit(String minSingleTimeLimit) {
		try {
			this.setMinSingleTimeLimit(Double.parseDouble(minSingleTimeLimit.trim()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	public Double getMaxSingleTimeLimit() {
		return maxSingleTimeLimit;
	}

	public void setMaxSingleTimeLimit(Double maxSingleTimeLimit) {
		this.maxSingleTimeLimit = maxSingleTimeLimit;
	}

	public void setMaxSingleTimeLimit(String maxSingleTimeLimit) {
		try {
			this.setMaxSingleTimeLimit(Double.parseDouble(maxSingleTimeLimit.trim()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	public Integer getMaxTotalTimeLimit() {
		return maxTotalTimeLimit;
	}

	public void setMaxTotalTimeLimit(Integer maxTotalTimeLimit) {
		this.maxTotalTimeLimit = maxTotalTimeLimit;
	}

	public void setMaxTotalTimeLimit(String maxTotalTimeLimit) {
		try {
			this.setMaxTotalTimeLimit(Integer.parseInt(maxTotalTimeLimit.trim()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

	public Integer getMaxImportProblemsPerDay() {
		return maxImportProblemsPerDay;
	}

	public void setMaxImportProblemsPerDay(Integer maxImportProblemsPerDay) {
		this.maxImportProblemsPerDay = maxImportProblemsPerDay;
	}

	public void setMaxImportProblemsPerDay(String maxImportProblemsPerDay) {
		try {
			this.setMaxImportProblemsPerDay(Integer.parseInt(maxImportProblemsPerDay.trim()));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public ServerConfig readServerConfig(URL serverUrl) throws DataException {
		logger.info("readServerConfig 讀取 " + serverUrl);
		...
		try {
			...
			e.printStackTrace();
			throw new DataException("無法讀取「裁判機參數」，未知錯誤！", e);
		}
	}
	...
	public boolean getCheckLocker(URL url, String KEY) throws Exception {
		...
		try {
			...
			e.printStackTrace();
			throw new DataException("CheckLocker: 無法連接裁判機，可能是裁判機有誤或網路不通！請管理員檢查。", e);
		}
	...
	public boolean isCaptchaValid(String secretKey, String response, IpAddress remoteAddr) {
	 ...
	 * @return
	 */
	public boolean getIsGoogleRecaptchaSetup(String remoteAddr) {
		IpAddress ip = new IpAddress(remoteAddr);
		if (ip == null || ip.isLoopbackAddress()) {
			return false;
		}
		if (this.getEnableReCaptcha() && !new AppConfig().getSite_key().equals(this.getSite_key())
				&& !new AppConfig().getSecret_key().equals(this.getSecret_key())) {
			return true;
		}
		return false;
	}
	...
	public boolean getIsGoogleLoginSetup() {
		if (this.getEnableGoogleLogin() && !new AppConfig().getClient_id().equals(this.getClient_id())
