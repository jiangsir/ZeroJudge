...
public class AppConfig {
	...
	private Locale[] locales = new Locale[] { new Locale("en", "US"), new Locale("zh", "TW"),
			new Locale("zh", "CN") };
...
//		this.setEnableMailer(Boolean.valueOf(enableMailer));
	...
	public void setProblemtabs(String problemtabs) {
		...
		try {
			this.setProblemtabs((ArrayList<Problemtab>) mapper.readValue(problemtabs,
					new TypeReference<ArrayList<Problemtab>>() {
					}));
		} catch (JsonParseException e) {
			e.printStackTrace();
			throw new DataException(
					" KEY \"" + FIELD.problemtabs + "\" " + e.getLocalizedMessage());
	...
	public void setTitle(String title) {
		...
		if (title.getBytes().length > MAX) {
			throw new DataException(
					"title 設定值超過長度限制 (" + title.getBytes().length + ">" + MAX + "字元)。");
		}
		this.title = title;
	}
	...
	public String getTitleImageBase64() throws IOException {
		if (this.getTitleImage().length == 0) {
			File titleImageFile = new File(
					ApplicationScope.getAppRoot() + File.separator + "images", "TitleImage.svg");
	...
	public String getLogoBase64() throws IOException {
		if (this.getLogo().length == 0) {
			File logoFile = new File(ApplicationScope.getAppRoot() + File.separator + "images",
					"Logo.svg");
			this.setLogo(FileUtils.readFileToByteArray(logoFile));
		}
		return "data:" + this.getLogoFileType() + ";base64,"
				+ new Base64().encodeToString(this.getLogo());
	}
	...
	public void setExclusiveSchoolids(String exclusiveSchoolids) {
		...
		this.setExclusiveSchoolids(
				(TreeSet<Integer>) StringTool.String2IntegerSet(exclusiveSchoolids));
	}
	...
	public void setBanners(String banners) {
		...
		try {
			this.setBanners((ArrayList<Banner>) mapper.readValue(banners,
					new TypeReference<ArrayList<Banner>>() {
					}));
		} catch (JsonParseException e) {
			e.printStackTrace();
		} catch (JsonMappingException e) {
	...
	public void setLast_solutionid_for_RecountProblemidset(
			Integer last_solutionid_for_RecountProblemidset) {
		this.last_solutionid_for_RecountProblemidset = last_solutionid_for_RecountProblemidset;
	}

	public void setLast_solutionid_for_RecountProblemidset(
			String last_solutionid_for_RecountProblemidset) {
		if (last_solutionid_for_RecountProblemidset == null
				|| !last_solutionid_for_RecountProblemidset.matches("[0-9]+")) {
			return;
		}
		this.setLast_solutionid_for_RecountProblemidset(
				Integer.parseInt(last_solutionid_for_RecountProblemidset));
	}
	...
	private String read(URL url, int timeoutSec) throws DataException {
		...
		try {
			...
			if (responsecode == 200) {
				...
				throw new DataException(
						"無法連接 URL : " + url.toString() + ", responsecode=" + responsecode);
			}
		} catch (Exception e) {
			e.printStackTrace();
			readCount++;
			...
			throw new DataException(
					"無法讀取裁判機的資料。(" + url.getHost() + " " + e.getLocalizedMessage() + ")", debugs);
		}
		try {
	...
	public ServerConfig getServerConfig() {
		...
		logger.info("this.serverConfig=" + this.serverConfig + " | this.serverConfig.getIsNull()="
				+ this.serverConfig.getIsNull() + " | this.serverConfig="
				+ (this.serverConfig != null));
		AppConfig appConfig = ApplicationScope.getAppConfig();
		try {
			if (readCount <= 1) {
				readCount++;
				logger.severe(new Date(System.currentTimeMillis())
						+ " !!!!! serverConfig==null !!!!，重新讀取(#" + readCount + ")。");
				ServerConfig serverConfig = appConfig.readServerConfig(serverUrl);
				appConfig.setServerConfig(serverConfig);
			} else {
				logger.severe(new Date(System.currentTimeMillis())
						+ " !!!!! serverConfig==newServerConfig !!!!，重新讀取(#" + readCount + ")。");
	...
	public boolean getCheckNopassRsync(URL serverUrl) {
		...
		run.exec(RunCommand.Command.sudo + " -u " + appConfig.getRsyncAccount() + " "
				+ RunCommand.Command.ssh + " -p " + serverConfig.getSshport() + " "
				+ serverConfig.getRsyncAccount() + "@" + serverUrl.getHost() + " \""
	...
	public boolean getCheckAllowedIP(String urlstring) {
		...
		try {
			...
			Logger.getAnonymousLogger()
					.info("STEP1: url.getDefaultPort()= " + url.getDefaultPort());
			...
			Logger.getAnonymousLogger()
					.info("STEP1: url.getContent()= " + url.getContent().toString());
	...
	public boolean getIsGoogleLoginSetup() {
		if (this.getEnableGoogleLogin()
				&& !new AppConfig().getClient_id().equals(this.getClient_id())
