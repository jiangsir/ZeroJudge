...
public class JudgeServer {
	...
	private URLConnection sendServerInput(URL url, JudgeObject judgeObject) throws DataException {
		...
		try {
			...
			double totaltimelimit_sec = 0;
			for (double timelimit : judgeObject.getProblem().getTimelimits()) {
				double timeextension = 1.0;
				for (Compiler compiler : appConfig.getServerConfig().getCompilers()) {
					if (compiler.getLanguage().equals(judgeObject.getLanguage().getName())) {
						timeextension = compiler.getTimeextension();
						break;
					}
				}

				totaltimelimit_sec += Math.ceil(timelimit * timeextension) * 2 + 2.0;
			}

			int timeout_sec = (int) (totaltimelimit_sec + 10);
		...
		logger.info("!!!!!!!!!!judgeObject.getProblem().getJudgemode()=" + judgeObject.getProblem().getJudgemode());
			...
			if (e instanceof java.io.IOException) {
				...
				if (e instanceof java.net.SocketTimeoutException) {
					serverOutputs[0].setHint(
							"讀取裁判機逾時！(" + hint + ")！讀取時間：" + (System.currentTimeMillis() - start) / 1000 + "s");
				} else {
					serverOutputs[0].setHint(
							"程式終止(" + e.getClass().getName() + ")！" + hint.replaceAll(conn.getURL().toString(), ""));
				}
