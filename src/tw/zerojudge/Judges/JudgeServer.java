...
public class JudgeServer {
	...
	private URLConnection sendServerInput(URL url, JudgeObject judgeObject) throws DataException {
		...
		for (int i = 0; i < testfiles.length; i++) {
			if (serverinput.getPriority() == JudgeObject.PRIORITY.Testjudge) {
				testfiles[i] = judgeObject.getProblem()
						.getTESTJUDGE_FILENAME(judgeObject.getSolution().getId(), i);
			} else {
	...
	private ServerOutput[] readServerOutputs(URLConnection conn) throws DataException {
		...
		try {
			...
			if (e instanceof java.io.IOException) {
				...
				serverOutputs[0].setHint(
						"無法讀取裁判機，請確認與裁判機加密鎖是否相符！" + hint.replaceAll(conn.getURL().toString(), ""));
		...
		try {
			serverOutputs = mapper.readValue(new String(outputString.getBytes("UTF-8"), "UTF-8"),
					ServerOutput[].class);
			return serverOutputs;
		} catch (JsonParseException e) {
			e.printStackTrace();
			throw new DataException(
					"評分資訊 (ServerInput) 格式有誤，可能使用了不匹配的裁判機系統。\n" + e.getLocalizedMessage(), e);
	...
	public synchronized void doJudge() throws DataException {
		...
		try {
			...
			summaryServerOutput(judgeObject.getPriority(), judgeObject.getProblem(),
					judgeObject.getSolution(), serverOutputs);
			notify();
	 ...
	 * @throws DataException
	 */
	public synchronized void summaryServerOutput(JudgeObject.PRIORITY priority, Problem problem,
			Solution solution, ServerOutput[] serverOutputs) {
		...
		} else if (priority == JudgeObject.PRIORITY.Submit
		...
		if (solution.getVclassid() > 0 && summary.getJudgement() == ServerOutput.JUDGEMENT.AC) {
			...
			VClassStudent student = vclassstudentDao.getStudent(solution.getVclassid(),
					solution.getUserid());
	...
	private void rebuiltUser(JudgeObject.PRIORITY priority, Solution solution) {
		if (priority == JudgeObject.PRIORITY.Submit || priority == JudgeObject.PRIORITY.Rejudge
			...
			new UserService().rebuiltUserStatisticByDataBase(
					new UserService().getUserById(solution.getUserid()));
		}
	}
	 ...
	 */
	private void fromTestcode(Solution solution, ServerOutput summary,
			ServerOutput[] serverOutputs) {
	...
	private void fromPrejudge(Problem problem, ServerOutput summary, ServerOutput[] serverOutputs) {
		...
		log.setMessage(problem.getProblemid() + ". " + problem.getTitle() + "(owner:"
				+ problem.getOwner() + ") 進行前測 -- " + problem.getPrejudgement());
