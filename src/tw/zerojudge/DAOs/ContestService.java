...
public class ContestService {
	...
	public void ImportContestByJson(OnlineUser onlineUser, File file, int targetVclassid)
		...
		prev_contest.setVclassid(targetVclassid);
		prev_contest.setVisible(VISIBLE.hide);
		prev_contest.setOwnerid(vclass.getOwnerid());
		...
		for (ExportProblem orig_Problem : exportContest.getExportProblems()) {
			Problem newproblem = problemService.InsertProblemByExportProblem(onlineUser, orig_Problem);
		...
		for (Solution orig_solution : exportContest.getSolutions()) {
			try {
				Logger.getAnonymousLogger().info("step3: prev_solution=" + orig_solution);
				orig_solution.setContestid(newcontestid);
				Logger.getAnonymousLogger().info("step4: origpids_newpids=" + origpids_newpids);
				Logger.getAnonymousLogger().info("step4: prev_solution.getPid()=" + orig_solution.getPid());
