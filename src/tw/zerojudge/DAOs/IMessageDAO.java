...
public class IMessageDAO extends SuperDAO<IMessage> {
	...
	public void sendIMessage(IMessage imessage, String hostURI) {
		...
		if (id > 0 && userto.getId().intValue() != 0 && userfrom.getId().intValue() != 0) {
			...
			content += "<h1>[" + ApplicationScope.getAppConfig().getTitle() + "("
					+ ApplicationScope.getHostURI() + ")]</h1>";
			content += "您好：<br><br>" + "請記得回覆站內信！<br><br>";
			content += "主旨：" + imessage.getSubject() + "<br>";
			content += "內容：<br><br><pre>" + imessage.getContent() + "</pre><br><br><br>";
			content += "<div>請注意！這是系統通知信，「請勿」直接回信！請到「站內訊息收件夾」進行回覆！！</div>";
			content += "</body></html>";

			Thread mailer = new Thread(new Mailer(userto, "站內訊息通知：" + userfrom.getAccount() + "("
					+ userfrom.getUsername() + ") 發送站內訊息給您。", content, hostURI));
			mailer.start();
		}
	}
	...
	public ArrayList<IMessage> getUnreadIMessage(OnlineUser onlineUser, int page) {
		String SQL = "SELECT * FROM imessages WHERE receiver='" + onlineUser.getAccount()
				+ "' AND receiver_status='" + IMessage.RECEIVER_STATUS.unread.name()
				+ "' AND receiver_status!='" + IMessage.RECEIVER_STATUS.deleted.name()
				+ "' AND sender_status='" + IMessage.SENDER_STATUS.open
	...
	public ArrayList<IMessage> getSentIMessages(OnlineUser onlineUser, int page) {
		String SQL = "SELECT * FROM imessages WHERE sender='" + onlineUser.getAccount()
				+ "' AND sender_status='" + IMessage.SENDER_STATUS.open
				+ "' ORDER BY senttime DESC LIMIT "
				+ (page - 1) * ApplicationScope.getAppConfig().getPageSize() + ","
				+ ApplicationScope.getAppConfig().getPageSize();
