...
public class SolutionService {
	...
	/**
	 * 進資料庫取得 userid 的最後一筆 solutionid xxxx 未完成
	 * 
	 * @param userid
	 * @return
	 */
	public Timestamp getLastSubmittimeByUserid(int userid) {
		return new SolutionDAO().getLastSubmittimeByUserid(userid);
	}
	 ...
	 * @return
	 */
	public ArrayList<Solution> getLatestSolutions(int n) {
		ArrayList<Solution> solutions = new ArrayList<Solution>();
		TreeSet<String> rules = new TreeSet<String>();
		rules.add("visible=" + Solution.VISIBLE.open.getValue());
		for (long solutionid : new SolutionDAO().getSolutionidsByRules(rules, "id DESC", 1)) {
	 ...
	 * @return
	 */
	public int getCountSolutionsByUseridContestid(int userid, int contestid) {
		TreeSet<String> rules = new TreeSet<String>();
