...
//		int loginlogid = new LoginlogDAO().insert(loginlog);
	...
	public boolean isReservedAccount(String account) {
		...
		if ("all".equals(account) || account.matches(".*system.*")
				|| account.matches(".*zerojudge.*") || "null".equals(account)) {
			return true;
		}
		return false;
	}
	...
	public void rebuiltUserStatisticByDataBase(User user) {
		...
		ArrayList<HashMap<String, Object>> list = new SolutionDAO()
				.getHashMapForUserStatistic_SLOW(user.getId());
		if (list == null) {
			return;
		}
