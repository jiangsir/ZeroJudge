...
public class UserService {
	...
	public final int maxMultiLogin = 10; 
	...
	public User getUserByAccount(String account) {
		if (account != null && HashAccountUserid.containsKey(account)) {
	...
	/**
	 * 在 CLASSMODE 時，使用 xuehao 搭配 school 來取得 user。
	 * 
	 * @param xuehao
	 * @param schoolid
	 * @return
	 */
	public User getUserByXuehaoSchoolid(String xuehao, String schoolid) {
		int int_schoolid = 0;
		try {
			int_schoolid = Integer.parseInt(schoolid);
		} catch (NumberFormatException e) {
		}
		User user = new UserDAO().getUserByAccountSchoolid(xuehao, int_schoolid);
		if (user == null || user.isNullUser()) {
			throw new InfoException("使用者不存在(account='" + xuehao + "', schoolid='" + schoolid + "')！");
		}
		return user;
	}

	/**
	 * 在 CLASSMODE 時，使用 xuehao 搭配 school 來登入。
	 * 
	 * @param xuehao
	 * @param schoolid
	 * @return
	 */
	public User getUserByXuehaoSchoolidPasswd(String xuehao, String schoolid, String passwd) {
		User user;
		if (schoolid == null || "".equals(schoolid) || schoolid.equals("-1") || !schoolid.matches("[0-9]+")) {
			user = new UserService().getUserByAccountPasswd(xuehao, passwd);
		} else {
			user = new UserDAO().getUserByAccountSchoolidPasswd(xuehao, Integer.parseInt(schoolid), passwd);
		}
		if (user == null || user.isNullUser()) {
			throw new InfoException("帳號或密碼可能有誤！");
		}
	...
	public ArrayList<User> getUsersByKeyword(String keyword) {
		...
		return new UserDAO().getUsersByKeyword(keyword);
	}
	...
	/**
	 * 專用於 ImportContest 的 insert User, 允許 @ 作為 user account
	 * 
	 * @param user
	 * @return
	 * @throws DataException
	 */
	public int insertByImportContest(User user) throws DataException {
		int userid = new UserDAO().insert(user);
		user.setId(userid);
		HashUsers.put(user.getId(), user);
		if (userid == 0) {
	...
	/**
	 * newuser 是更新了部分資料的 user
	 * 
	 * @param newuser
	 */
	public void refreshOnlineUser(User newuser) {
		/**
		 * 更新"學生註解"時，一併更新 OnlineUser
		 */
		OnlineUser newonlineUser = null;
		for (OnlineUser onlineUser : ApplicationScope.getOnlineUsers().values()) {
			if (newuser.getAccount().equals(onlineUser.getAccount())) {
				HttpSession session = SessionFactory.getSessionById(onlineUser.getSessionid());
				if (session == null) {
					break;
				}
				newonlineUser = new OnlineUser(session, newuser);
				SessionScope sessionScopeOnlineUser = new SessionScope(session);
				sessionScopeOnlineUser.setSessionid(onlineUser.getSessionid());
				sessionScopeOnlineUser.setOnlineUser(newonlineUser);
				sessionScopeOnlineUser.setUnreadIMessages(new IMessageDAO().getUnreadIMessage(newonlineUser, 1));
				break;
			}
		}
		if (newonlineUser != null) {
			ApplicationScope.getOnlineUsers().put(newonlineUser.getSessionid(), newonlineUser);
		}
	...
	public void rebuiltUserStatisticByDataBase(User user) {
		...
		logger.info("(耗時 " + (System.currentTimeMillis() - starttime) + " ms)" + this.getClass().getName()
				+ ".rebuiltUserStaticByDatabase(" + user + ")");
	}
	...
	public ArrayList<User> getUsersByROLEUSER(int pagenum) {
		return new UserDAO().getUsersByRoleOrderById(new User.ROLE[] { User.ROLE.USER, User.ROLE.MANAGER }, pagenum);
	}

	public ArrayList<User> getUsersByROLEDEBUGGER() {
		return new UserDAO().getUsersByRoleOrderById(new User.ROLE[] { User.ROLE.DEBUGGER }, 1);
	}
