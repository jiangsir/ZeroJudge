...
public class ProblemService {
	...
	public int update(Problem problem) {
		...
		if ((oldProblem.getDisplay() == Problem.DISPLAY.open
				&& problem.getDisplay() != Problem.DISPLAY.open)
				|| (oldProblem.getDisplay() != Problem.DISPLAY.open
						&& problem.getDisplay() == Problem.DISPLAY.open)) {
	...
	public Hashtable<String, TreeSet<Problemid>> getProblemtabProblemidset() {
		if (ProblemService.HashProblemtabProblemidset.size() == 0) {
			...
			for (Problemtab problemtab : appConfig.getProblemtabs()) {
				ProblemService.HashProblemtabProblemidset.put(problemtab.getId(), this
						.getProblemidSetByTabidDisplay(problemtab.getId(), Problem.DISPLAY.open));
			}
		}
		return HashProblemtabProblemidset;
	}

	private TreeSet<Problemid> getProblemidSetByTabidDisplay(String tabid,
	...
	public Problemid createNextProblemid() {
		...
		for (char prefix = ApplicationScope.getAppConfig().getProblemid_prefix()
				.charAt(0); prefix <= 'z'; prefix++) {
	...
	public Problem ImportProblemJson(OnlineUser onlineUser, File file)
		...
		for (Problemimage image : images) {
			...
			problem.setTheoutput(
					this.replaceImageForImport(oldid, imageid, problem.getTheoutput()));
	...
	public Problem downloadProblemAttachfiles(Problem problem, int serverport) {
		problem.setContent(
				this.downloadImage(problem.getProblemid(), problem.getContent(), serverport));
		problem.setContent(
				this.downloadAudio(problem.getProblemid(), problem.getContent(), serverport));
		problem.setTheinput(
				this.downloadImage(problem.getProblemid(), problem.getTheinput(), serverport));
		problem.setTheinput(
				this.downloadAudio(problem.getProblemid(), problem.getTheinput(), serverport));
		problem.setTheoutput(
				this.downloadImage(problem.getProblemid(), problem.getTheoutput(), serverport));
		problem.setTheoutput(
				this.downloadAudio(problem.getProblemid(), problem.getTheoutput(), serverport));
	...
	private String downloadImage(Problemid problemid, String html, int serverport) {
		...
		for (Element img : imglist) {
			...
			String urlpattern = ShowImageServlet.class.getAnnotation(WebServlet.class)
					.urlPatterns()[0];
			urlpattern = urlpattern.substring(1);
	 ...
	 * @return
	 */
	private void replaceAudioSrc(OutputDocument document, Element audio, String urlpattern,
			int id) {
		String src = audio.getAttributeValue("src");
		if (src != null) {
			document.replace(audio.getAttributes().get("src").getValueSegment(),
					urlpattern + "?id=" + id);
			return;
		}
		Element audio_source = audio.getFirstElement(HTMLElementName.SOURCE);
		if (audio_source != null) {
			document.replace(audio_source.getAttributes().get("src").getValueSegment(),
					urlpattern + "?id=" + id);
			return;
		}
	}
	 ...
	 * @return
	 */
	public ArrayList<Problem> getEditableProblems(OnlineUser onlineUser, TreeSet<String> rules,
...
//			fields.put("display!", Problem.DISPLAY.unfinished.name());
	...
	public ArrayList<Problem> getAllProlems(OnlineUser onlineUser, String orderby) {
		if (onlineUser.getIsDEBUGGER()) {
			ArrayList<Problem> problems = new ProblemDAO()
					.getProblemByFields(new TreeMap<String, Object>(), orderby, 0);
			return problems;
		} else {
			return new ArrayList<Problem>();
		}
	 ...
	 * @return
	 */
	public ArrayList<Problem> getOpenProblemsByRules(TreeSet<String> rules, String orderby,
			int page) {
		rules.add("display='" + Problem.DISPLAY.open.name() + "'");
		return new ProblemDAO().getProblemsByRules(rules, orderby, page);
	}

	public ArrayList<Problem> getOpenProblemsByFields(TreeMap<String, Object> fields,
			String orderby, int page) {
		fields.put("display", Problem.DISPLAY.open.name());
	...
	public void doPreJudge(OnlineUser onlineUser, Problem problem) {
		...
		try {
			...
			new JudgeServer().summaryServerOutput(JudgeObject.PRIORITY.Prejudge, problem, null,
					serverOutputs);
			e.printStackTrace();
			return;
		}
	 ...
	 * @throws IOException
	 */
	public void doCreateTestfile(Problem problem, int index, String infiledata, String outfiledata)
			throws IOException {
		AppConfig appConfig = ApplicationScope.getAppConfig();
...
//		new RunCommand().exec("sudo chown -R " + rsyncaccount + ":" + rsyncaccount + " "
//				+ appConfig.getTestdataPath(problem.getProblemid()).getAbsolutePath());
//		new RunCommand().exec("sudo chmod 777 " + appConfig.getTestdataPath(problem.getProblemid()).getAbsolutePath());
	}

	/**
	 * 刪除 problem.timelimits 其中一個 index
	 * 
	 * @return
	 */
		...
		if (filein.exists() && fileout.exists()) {
			...
			for (int i = index; i < testfilelength; i++) {
				new File(appConfig.getTestdataPath(problem.getProblemid()),
						problem.getTESTDATA_FILENAME(i + 1) + ".in").renameTo(
								new File(appConfig.getTestdataPath(problem.getProblemid()),
										problem.getTESTDATA_FILENAME(i) + ".in"));
				new File(appConfig.getTestdataPath(problem.getProblemid()),
						problem.getTESTDATA_FILENAME(i + 1) + ".out").renameTo(
								new File(appConfig.getTestdataPath(problem.getProblemid()),
	...
	public void rsyncAllTestdatas() {
		...
		if (!testdataPath.exists()) {
			...
			throw new DataException("本地端的測資(" + testdataPath.getName() + ")不存在，因此無法同步到裁判機上！請管理員檢查。",
					debugs);
		}
		...
		rsync += "rsync -av --delete -e \"ssh -p " + appConfig.getServerConfig().getSshport()
				+ "\" " + appConfig.getTestdataPath() + File.separator + " "
				+ appConfig.getServerConfig().getRsyncAccount() + "@"
		...
		if (runCommand.getErrStream().size() != 0) {
			throw new DataException("無法與裁判機同步測資(rsyncAllTestdatas)！請管理員檢查。\n",
	...
	public void rsyncTestdataByProblem(Problem problem, OnlineUser onlineUser) {
		...
		if (!testfile.exists()) {
			...
			logger.info("本地端的測資檔不存在，因此無法同步到裁判機上！請管理員檢查。(" + testfile + ")");
			throw new DataException("本地端的測資檔不存在，因此無法同步到裁判機上！請管理員檢查。(" + testfile.getName() + ")",
					debugs);
		}
		...
		rsync += RunCommand.Command.rsync + " -av --delete -e \"" + RunCommand.Command.ssh + " -p "
				+ appConfig.getServerConfig().getSshport() + "\" "
				+ appConfig.getTestdataPath(problem.getProblemid()) + " ";
		InetAddress address;
		try {
			address = InetAddress.getByName(appConfig.getServerUrl().getHost());
			if (!address.isLoopbackAddress()) {
				rsync += appConfig.getServerConfig().getRsyncAccount() + "@"
						+ appConfig.getServerUrl().getHost() + ":";
			}
		...
		if (runCommand.getErrStream().size() != 0) {
			...
			String title = "無法與裁判機同步測資(rsyncTestdataByProblem)！請管理員檢查。";
...
//			}
		...
		if (!specialjudge.exists()) {
			...
			throw new DataException("本地端的「Special Judge 資料夾」建立失敗，因此無法同步到裁判機上！請管理員檢查。("
					+ specialjudge.getName() + ")", debugs);
		}
		...
		if (runCommand.getErrStream().size() != 0) {
			...
			debugs.add(rsync);
			debugs.addAll(runCommand.getErrStream());
			String title = "無法與裁判機同步「Special Judge程式」！請管理員檢查。";
			String subtitle = "";
			String content = "";
			Alert alert = new Alert(Alert.TYPE.DATAERROR, title, subtitle, content, null, debugs,
					onlineUser);
			throw new DataException(alert);
		}
	 ...
	 * @return
	 */
	public TreeSet<String> getSearchRules(String tag, String[] tab_search, String[] difficulty,
			String keyword, String background, String searchword, String locale, String ownerid) {
		...
		if (tag != null && !"".equals(tag.trim())) {
			...
			rules.add("keywords LIKE '%" + tag + "%' OR backgrounds LIKE '%" + tag
		...
		if (searchword != null && !"".equals(searchword)) {
			...
			rules.add("problemid='" + searchword + "' OR title LIKE '%" + searchword
					+ "%' OR backgrounds LIKE '%" + searchword + "%' OR content LIKE '%"
					+ searchword + "%' OR theinput LIKE '%" + searchword + "%' OR theoutput LIKE '%"
					+ searchword + "%' OR hint LIKE '%" + searchword + "%' OR reference LIKE '%"
		...
		if (keyword != null && !"".equals(keyword)) {
			...
			rules.add("keywords LIKE '%" + keyword + "%' OR backgrounds LIKE '%" + keyword
