...
public class ProblemService {
	...
	public Problem getProblemByPid(int pid) {
		...
		logger.info("ProblemService.getProblemByPid(): pid=" + pid);
		if (HashProblems.containsKey(pid)) {
			if (HashProblems.get(pid).isNullProblem()) {
				logger.warning("HashProblems 發現 NullProblem pid=" + pid + ", problemid()="
						+ HashProblems.get(pid).getProblemid());
			} else {
	...
	public Problem getProblemByProblemid(Problemid problemid) {
		...
		int pid = 0;
		if (HashProblemids.containsKey(problemid) && HashProblemids.get(problemid).intValue() != 0) {
			...
			if (pid != 0) {
				...
				logger.warning("ProblemDAO().getIdByProblemid 取得 pid==0 錯誤!!。problemid=" + problemid);
			}
		}
		logger.info("ProblemService.getProblemByProblemid(): problemid=" + problemid + ", pid=" + pid);
	...
	/**
	 * 從 ExportProblem Object 新增一個新的 Problem
	 * 
	 * @param onlineUser
	 * @param exportProblem
	 * @return
	 * @throws IOException
	 */
	public Problem InsertProblemByExportProblem(OnlineUser onlineUser, ExportProblem exportProblem) throws IOException {
		...
		try {
			problem.checkTimelimits(exportProblem.getTimelimits());
			problem.setTimelimits(exportProblem.getTimelimits());
		} catch (Exception e) {
			double[] timelimits = exportProblem.getTimelimits();
	 ...
	 * @throws IOException
	 */
	public Problem ImportProblemByJsonfile(OnlineUser onlineUser, File file)
			throws JsonParseException, JsonMappingException, IOException {
		ObjectMapper mapper = new ObjectMapper(); 
		ExportProblem exportProblem = mapper.readValue(file, ExportProblem.class);
		return this.InsertProblemByExportProblem(onlineUser, exportProblem);
	}

	/**
	 * 搜尋某 author 24小時內 problem 數。
	 * 
	 * @param onlineUser
	 * @return
	 */
	public int getCountBy24HourNewProblems(OnlineUser onlineUser) {
		return new ProblemDAO().getCountBy24HourNewProblems(onlineUser.getId());
	}
	...
	public void insertInitProblems() {
		...
		problem.setHint("<p>程式說明：讀取測試資料，請參考 <a href=\"UserGuide.html#Samplecode\">" + "範例程式碼</a></p>");
	...
	public void rsyncTestdataByProblem(Problem problem, OnlineUser onlineUser) {
		...
		String rsync = "sudo ";
		rsync += ApplicationScope.getAppConfig().getBinPath() + File.separator + "rsyncTestdataByProblem.py ";
		rsync += appConfig.getRsyncAccount() + " "; 
		rsync += appConfig.getServerConfig().getSshport() + " "; 
		rsync += appConfig.getTestdataPath(problem.getProblemid()) + " "; 
		rsync += this.isLoopback(appConfig.getServerUrl().getHost()) + " "; 
		rsync += appConfig.getServerConfig().getRsyncAccount() + " "; 
		rsync += appConfig.getServerUrl().getHost() + " "; 
		rsync += appConfig.getServerConfig().getTestdataPath(); 
	...
	public void rsyncSpecialJudge(Problem problem, OnlineUser onlineUser) {
		...
		String rsynccmd = "sudo ";
		rsynccmd += appConfig.getBinPath() + File.separator + "rsyncSpecialJudge.py ";
		rsynccmd += appConfig.getRsyncAccount() + " "; 
		rsynccmd += appConfig.getServerConfig().getSshport() + " "; 
		rsynccmd += appConfig.getSpecialPath(problem.getProblemid()) + " "; 
		try {
			rsynccmd += InetAddress.getByName(appConfig.getServerUrl().getHost()).isLoopbackAddress() + " ";
		} catch (UnknownHostException e1) {
			e1.printStackTrace();
		}
		rsynccmd += appConfig.getServerConfig().getRsyncAccount() + " ";
		rsynccmd += appConfig.getServerUrl().getHost() + " ";
		rsynccmd += appConfig.getServerConfig().getSpecialPath(); 


		RunCommand runCommand = new RunCommand();
		runCommand.exec(rsynccmd);
	...
	public TreeSet<String> getSearchRules(String tag, String[] tab_search, String[] difficulty, String keyword,
		...
		if (searchword != null && !"".equals(searchword)) {
			if (searchword.length() > 30) {
				throw new DataException("searchword 太長！");
			}
