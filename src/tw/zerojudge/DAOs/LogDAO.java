...
public class LogDAO extends SuperDAO<Log> {
	...
	public ArrayList<Log> getLogByTabid(TreeSet<Log.TABID> tabids, long timeslice)
		...
		try {
			...
			pstmt.setTimestamp(tabidarray.length + 1,
					new Timestamp(System.currentTimeMillis() - timeslice));
	...
	public boolean reduce(int left) {
		String sql = "DELETE FROM `logs` WHERE id < (SELECT id FROM (SELECT MAX(id) as id FROM `logs`) as tt)-"
				+ left;
