	...
	public AccessTokenExtractor getAccessTokenExtractor() {
		return new AccessTokenExtractor() {
			...
			public Token extract(String response) {
				...
				try {
					HashMap<?, ?> jsonMap = mapper.readValue(response, HashMap.class);
					if (jsonMap.containsKey("access_token")) {
						String token = OAuthEncoder.decode((String) jsonMap.get("access_token"));
					...
					e.printStackTrace();
					throw new OAuthException("JsonProcessingException: json 反序列化過程發生錯誤!", null);
				} catch (IOException e) {
					e.printStackTrace();
					throw new OAuthException("IOException: json 讀取發生錯誤!", null);
				}
