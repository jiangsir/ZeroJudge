	...
	private boolean isAccessible(HttpSession session) {
		...
		for (IpAddress session_ip : sessionScope.getSession_ipset()) {
			if ((sessionScope != null && sessionScope.getOnlineUser() != null
					&& sessionScope.getOnlineUser().getIsHigherEqualThanMANAGER())
					|| session_ip.getIsSubnetOf(ApplicationScope.getAppConfig().getManager_ip())) {
				return true;
			}
		}
		return false;
		...
		if (session == null) {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (session.getAttribute(name) instanceof OnlineUser) {
					OnlineUser onlineUser = (OnlineUser) session.getAttribute(name);
					content.append("<strong>" + name + "</strong> = " + onlineUser + "(" + onlineUser.getAccount() + ")"
							+ "<br>");
				} else {
					content.append("<strong>" + name + "</strong> = " + session.getAttribute(name) + "<br>");
				}
			}
		}
