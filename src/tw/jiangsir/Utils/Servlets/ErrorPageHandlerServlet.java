...
public class ErrorPageHandlerServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		if (!log.getUri().equals("/favicon.ico") && !log.getUri().equals("/robots.txt")
				&& !requestUri.matches(".*\\.png") && !requestUri.matches(".*RealtimeStatus")
				&& !requestUri.matches(".*\\.js") && !requestUri.matches(".*\\.css")
				&& !requestUri.matches(".*RealtimeStatu")
				&& !requestUri.matches(".*RealtimeStatus")) {
			new LogDAO().insert(log);
		}

		Alert alert = new Alert(Alert.TYPE.ERROR, message, log.getTitle(),
