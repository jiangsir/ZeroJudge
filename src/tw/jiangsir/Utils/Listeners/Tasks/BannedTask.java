...
public class BannedTask extends TimerTask {
	...
	public void run() {
		try {
			...
			TreeMap<IpAddress, ArrayList<SessionScope>> ipTreeMap = this.builtIPTreeMap();
	...
	public TreeMap<IpAddress, ArrayList<SessionScope>> builtIPTreeMap() {
		...
		for (HttpSession session : ApplicationScope.getOnlineSessions().values()) {
			try {
			...
			for (IpAddress session_ip : session_ipset) {
				...
				if (iptreeMap.containsKey(session_ip)) {
					...
					if (size >= appConfig.getMaxConnectionByIP() && !iptreeMap.containsKey(session_ip)) {
						...
						for (SessionScope ipsessionScope : iptreeMap.get(session_ip)) {
	...
	public List<Map.Entry<IpAddress, ArrayList<SessionScope>>> getSortedIPlist(
		...
		ArrayList<Map.Entry<IpAddress, ArrayList<SessionScope>>> list = new ArrayList<Map.Entry<IpAddress, ArrayList<SessionScope>>>(
				ipTreeMap.entrySet());
		Collections.sort(list, new Comparator<Map.Entry<IpAddress, ArrayList<SessionScope>>>() {
			public int compare(Entry<IpAddress, ArrayList<SessionScope>> o1,
