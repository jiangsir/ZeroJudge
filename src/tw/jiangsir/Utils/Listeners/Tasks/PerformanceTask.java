...
public class PerformanceTask extends TimerTask {
	...
	public void run() {
		...
		memory.append("記憶體用量：" + ApplicationScope.getMemoryInfo() + ", Solution Cache size="
				+ SolutionService.HashSolutions.size() + ", " + "User Cache size="
				+ UserService.HashUsers.size() + ", " + "Problem Cache size="
				+ ProblemService.HashProblems.size() + ", " + "Contest Cache size="
		...
		if (percent > 0.7 && percent < 0.9) {
			...
			log.setTitle("記憶體用量偏高(" + beforeInfo + ")，啟動 GC 減少 " + (beforeUsage - afterUsage)
					+ "MB 記憶體用量");
