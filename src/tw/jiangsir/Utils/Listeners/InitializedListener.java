...
public class InitializedListener implements ServletContextListener {
	...
	private void contestCloserInitialized(ServletContext context) {
		...
		for (Task task : taskDao.getTasksByContestCloser()) {
			...
			Thread contestCloser = new Thread(
					new ContestCloser(contest, task.getFirststart(), delay));
			contestCloser.start();
		}
	}
	...
	private void contestStarterInitialized(ServletContext context) {
		...
		for (Task task : taskDao.getTasksByContestStarter()) {
			...
			taskDao.update(task);
			Contest contest = new ContestDAO().getContestById(Integer.valueOf(parameter));
			Thread contestStarter = new Thread(
					new ContestStarter(contest, task.getFirststart(), delay));
	...
	public void contextInitialized(ServletContextEvent event) {
		try {
			...
			for (User.ROLE role : User.ROLE.values()) {
				ApplicationScope.getRoleMap().put(role,
						new HashSet<Class<? extends HttpServlet>>());
			}
			...
			for (String key : registrations.keySet()) {
				...
				try {
					...
					if (httpServlet instanceof HttpServlet) {
						...
						for (User.ROLE role : new Filter5_RoleFilter().getRoleSet(httpServlet)) {
							ApplicationScope.getRoleMap().get(role).add(httpServlet.getClass());
						}

						WebServlet webServlet = httpServlet.getClass()
...
//				new RunCommand().exec("sudo python3 "
//						+ ApplicationScope.getAppRoot().getAbsolutePath() + File.separator
//						+ "InitializedListener.py permission --rsyncaccount '"
//						+ appConfig.getRsyncAccount() + "' --CONSOLE '" + appConfig.getConsolePath()
//						+ "'");
				new RunCommand()
						.exec("sudo python3 " + ApplicationScope.getAppRoot().getAbsolutePath()
								+ File.separator + "InitializedListener.py "
								+ appConfig.getRsyncAccount() + " " + appConfig.getConsolePath());
			} else {
