...
public class RoleFilter implements Filter {
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		...
		case CLOSE_MODE:
			if (httpServlet.getClass() == LoginServlet.class
		...
		case CONTEST_MODE:
			if (!onlineUser.getIsHigherEqualThanMANAGER()) {
				...
				if (!servletSet.contains(httpServlet.getClass())) {
					response.sendRedirect(request.getContextPath() + ContestsServlet.class
		...
		default:
			...
			Alert alert = new Alert(Alert.TYPE.ROLEERROR, "系統模式有誤！", "SYSTEM_MODE 設定錯誤，請聯繫管理員修正。",
