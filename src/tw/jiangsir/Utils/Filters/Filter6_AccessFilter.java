 ...
 */
@WebFilter(filterName = "Filter6_AccessFilter", urlPatterns = { "/*" }, asyncSupported = true)
			...
			if (iface == IAccessFilter.class) {
				for (Method method : IAccessFilter.class.getMethods()) {
					try {
						...
						servletMethod.invoke(httpServlet.getClass().getDeclaredConstructor().newInstance(),
								new Object[] { request });
	...
	private boolean isAccessible(OnlineUser onlineUser, Class<? extends HttpServlet> httpServletClazz,
		...
		if (onlineUser.getDenyServlet().contains(httpServletClazz)) {
			if (onlineUser.isIsInContest()) {
				String title = "您(" + onlineUser.getAccount() + ")的權限("
						+ httpServletClazz.getAnnotation(WebServlet.class).urlPatterns()[0] + ") 暫時取消。";
				String content = "您正參與「<a href=\"ShowContest?contestid=" + onlineUser.getJoinedcontestid() + "\">"
						+ onlineUser.getJoinedContest().getTitle() + "</a>」當中，交卷後即可恢復。";

				Alert alert = new Alert(Alert.TYPE.ROLEERROR, title, "", content, null, null, onlineUser);
				throw new RoleException(alert);
			}
			throw new RoleException("您(" + onlineUser.getAccount() + ")的權限("
					+ httpServletClazz.getAnnotation(WebServlet.class).urlPatterns()[0] + ") 已經被暫時取消。");
		}
		if (!onlineUser.getAllowServlet().contains(httpServletClazz)) {
			throw new RoleException("您(" + onlineUser.getAccount() + ")沒有這個權限("
