...
public class Filter6_AccessFilter implements Filter {

	/**
	 * Default constructor.
	 */
	public Filter6_AccessFilter() {
	}
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		if (onlineUser != null && !onlineUser.isNullOnlineUser()
			...
			debugs.add(
					"本頁面 " + httpServlet.getClass().getAnnotation(WebServlet.class).urlPatterns()[0]
							+ " 瀏覽角色：");
			debugs.add("使用者：" + onlineUser.getAccount() + " 角色：" + onlineUser.getRole());
			debugs.add(
					"可用servlet 共" + ApplicationScope.getRoleMap().get(onlineUser.getRole()).size()
							+ "個:" + ApplicationScope.getRoleMap().get(onlineUser.getRole()));

			Alert alert = new Alert(Alert.TYPE.ROLEERROR,
					"您(" + onlineUser.getAccount() + ")沒有權限瀏覽這個頁面。",
					httpServlet.getClass().getAnnotation(WebServlet.class).urlPatterns()[0], "",
					null, debugs, onlineUser);
				...
				for (Method method : IAccessFilter.class.getMethods()) {
					try {
						Method servletMethod = httpServlet.getClass()
								.getDeclaredMethod(method.getName(), HttpServletRequest.class);
						servletMethod.invoke(httpServlet.getClass().newInstance(),
								new Object[] { request });
	 ...
	 * @throws Throwable
	 */
	private boolean isAccessible(OnlineUser onlineUser,
			Class<? extends HttpServlet> httpServletClazz, HttpServletRequest request) {
		...
		if (onlineUser.getDenyServlet().contains(httpServletClazz)) {
			throw new RoleException(
					"您的權限(" + httpServletClazz.getAnnotation(WebServlet.class).urlPatterns()[0]
							+ ") 已經被暫時取消。");
		}
		if (!onlineUser.getAllowServlet().contains(httpServletClazz)) {
			throw new RoleException("您沒有這個權限("
					+ httpServletClazz.getAnnotation(WebServlet.class).urlPatterns()[0] + ")");
		}
		return true;
