...
@WebFilter(filterName = "Filter5_RoleFilter", urlPatterns = { "/*" }, asyncSupported = true)
@Privilege
		...
		switch (appConfig.getSystemMode()) {
			...
			case CLASS_MODE:
				...
				if (!onlineUser.getIsHigherEqualThanMANAGER()) {
					...
					servletSet.add(InsertBatchedUsersServlet.class);
					...
					servletSet.add(ReJudgeAjaxServlet.class);
					servletSet.add(ImportContestServlet.class);					
					...
					servletSet.add(UploadFilesServlet.class);
			...
			case CONTEST_MODE:
				...
				if (!onlineUser.getIsHigherEqualThanMANAGER()) {
					...
					servletSet.add(EditContestsServlet.class);
					servletSet.add(InsertContestServlet.class);
					servletSet.add(ImportContestServlet.class);
