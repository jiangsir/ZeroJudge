 ...
 */
@WebFilter(filterName = "Filter7_ExceptionHandlerFilter", urlPatterns = { "/*" }, asyncSupported = true)
public class Filter7_ExceptionHandlerFilter implements Filter {
	Logger logger = Logger.getAnonymousLogger();

	/**
	 * Default constructor.
	 */
	public Filter7_ExceptionHandlerFilter() {
	}
	...
	public void doFilter(ServletRequest req, ServletResponse resp, FilterChain chain)
		...
		try {
			...
			logger.info("這個 Exception 由 " + this.getClass().getSimpleName() + " 所捕捉");
			ArrayList<String> list = alert.getStackTracelist();
			while (rootCause.getCause() != null) {
				list.add(rootCause.getClass().getSimpleName() + ": " + rootCause.getLocalizedMessage());
				rootCause = rootCause.getCause();
			}
			logger.info("這個 " + e + " 由 " + this.getClass().getSimpleName() + " 所捕捉");
			...
			if (e instanceof JQueryException) {
				...
				return;
			} else if (e instanceof JQuerySuccess) {
				response.setStatus(HttpServletResponse.SC_OK);

				alert = ((JQuerySuccess) e).getAlert();
				HashMap<String, String> jQueryJson = new HashMap<String, String>();
				jQueryJson.put("type", String.valueOf(alert.getType()));
				jQueryJson.put("title", alert.getTitle());
				ObjectMapper mapper = new ObjectMapper();
				response.getWriter().write(mapper.writeValueAsString(jQueryJson));
				response.flushBuffer();
				return;
				...
				log.setTabid(Log.TABID.EXCEPTIONS);
				if (e instanceof DataException) {
					...
					alert.getStackTracelist().addAll(checkCause.getChecks());
