...
public class ApplicationScope {
	...
	private static String hostURI = "unknown HOST";
	...
	public static void setOnlineSessionScopes(
			ConcurrentHashMap<String, HttpSession> onlineSessions) {
	...
	public static void setRoleMap(
			HashMap<User.ROLE, HashSet<Class<? extends HttpServlet>>> roleMap) {
		...
		try {
			ApplicationScope.version = FileUtils
					.readFileToString(new File(
							ApplicationScope.appRoot + File.separator + "META-INF", "Version.txt"))
					.trim();
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static void setIsReleased() {
		ApplicationScope.isReleased = getVersion().matches("[0-9]+\\.[0-9]+\\.[0-9]+");
		ApplicationScope.servletContext.setAttribute("isReleased", isReleased);
	}

	//

	public static AppConfig reloadAppConfig() {
		...
		return ApplicationScope.appConfig;
	}
	...
	public static void setHostURI(String hostURI) {
		ApplicationScope.hostURI = hostURI;
		ApplicationScope.servletContext.setAttribute("hostURI", ApplicationScope.hostURI);
	}
