...
import tw.zerojudge.Servlets.Utils.ChangePasswordServlet;
...
public class SessionScope implements Serializable {
	...
	private Locale session_locale = Problem.locale_zh_TW;
	...
	public SessionScope(HttpSession session) {
		...
		this.setSession_privilege(
				(LinkedHashSet<String>) session.getAttribute("session_privilege"));
		...
		this.setSession_requestheaders(
				(HashMap<String, String>) session.getAttribute("session_requestheaders"));
	...
	/**
	 * 判斷是否為 LoopbackAddress
	 * 
	 * @return
	 */
	public boolean getIsLoopbackAddress() {
		for (IpAddress ipAddress : this.getSession_ipset()) {
	...
	public void setHostURI(HttpServletRequest request) {
		...
		ApplicationScope.setHostURI(hostURI);
	}
	...
	public void setReturnPage(String servletPath, String querystring) {
		if (servletPath
				.startsWith(LoginServlet.class.getAnnotation(WebServlet.class).urlPatterns()[0])
				|| servletPath.startsWith(
						LogoutServlet.class.getAnnotation(WebServlet.class).urlPatterns()[0])
				|| servletPath.startsWith(ChangePasswordServlet.class
						.getAnnotation(WebServlet.class).urlPatterns()[0])
				|| servletPath.startsWith(
						GoogleLoginServlet.class.getAnnotation(WebServlet.class).urlPatterns()[0])
				|| servletPath.startsWith(
						UnbindGoogleServlet.class.getAnnotation(WebServlet.class).urlPatterns()[0])
				|| servletPath.startsWith(CallbackGoogleLoginServlet.class
						.getAnnotation(WebServlet.class).urlPatterns()[0])
				|| servletPath.startsWith(
						InsertUserServlet.class.getAnnotation(WebServlet.class).urlPatterns()[0])
				|| servletPath.startsWith(
						ShowSessionsServlet.class.getAnnotation(WebServlet.class).urlPatterns()[0])
				|| servletPath.startsWith(
						ShowImageServlet.class.getAnnotation(WebServlet.class).urlPatterns()[0])
				|| servletPath.startsWith("/Update") || servletPath.startsWith("/Insert")
				|| servletPath.startsWith("/api/") || servletPath.endsWith(".ajax")
				|| servletPath.endsWith(".api") || servletPath.endsWith(".json")) {
			return;
		}
	...
	public void setIsOnlineUser() {
		this.isOnlineUser = this.getOnlineUser() != null
				&& !this.getOnlineUser().isNullOnlineUser();
