...
public class ContestTLD {
	...
	public static boolean isCanShowContestNote(OnlineUser onlineUser, Contest contest) {
		...
		if (onlineUser.getJoinedcontestid().intValue() != contest.getId()
				&& !contest.getIsStopped()) {
			return true;
		}
		return false;
	}
...
//	private static boolean isVisible_FinishContest(OnlineUser onlineUser, Contest contest) {
//		if (onlineUser == null) {
//			onlineUser = UserFactory.getNullOnlineUser();
//		}
//		if ((contest.getIsRunning() || contest.getIsStarting()) && (onlineUser.getIsDEBUGGER()
//				|| contest.getIsOwner(onlineUser)
...
//	}
	...
	public static boolean isVisible_pauseContest(OnlineUser onlineUser, Contest contest) {
		...
		if ((onlineUser.getIsDEBUGGER() || contest.getIsOwner(onlineUser))
				&& (contest.getIsRunning()) && contest.getProblemids().size() != 0) {
			return true;
		}
		return false;
	}
	...
	public static boolean isVisible_resumeContest(OnlineUser onlineUser, Contest contest) {
		...
		if ((onlineUser.getIsDEBUGGER() || contest.getIsOwner(onlineUser))
				&& (contest.getIsPausing()) && contest.getProblemids().size() != 0) {
			return true;
		}
		return false;
	}
...
//		}
		try {
			...
			new EditContestantsServlet().AccessFilter(onlineUser, contest);
			return true;
		} catch (AccessException e) {
			return false;
		}
	...
	public static boolean getIsStudent(OnlineUser onlineUser, Contest contest) {
		...
		if (new VClassStudentDAO().getStudent(onlineUser.getVclassid(),
				onlineUser.getId()) == null) {
			return false;
		}
		return true;
	}
	...
	public static String getResource_Countdown(Locale locale, Contest contest) {
		...
		s.append(mins % 60 + " " + resource.getString(Message.Resource_Codelock_Minute) + " "
				+ secs % 60 + " " + resource.getString(Message.Resource_Codelock_Second));
		return s.toString();
	}
