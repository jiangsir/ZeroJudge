...
public class HtmlUtils {
	 ...
	 * @return
	 */
	private static String sanitizeHtml_OLD(String htmlContent) {
	...
	private static final PolicyFactory IMAGES = (new HtmlPolicyBuilder()).allowUrlProtocols("http", "https", "data")
			.allowElements("img").allowAttributes("alt", "src").onElements("img")
			.allowAttributes("border", "height", "width").onElements("img").toFactory();

	private static final PolicyFactory LINKS = (new HtmlPolicyBuilder()).allowStandardUrlProtocols().allowElements("a")
			.allowAttributes("href", "target").onElements("a").requireRelNofollowOnLinks().toFactory();

	private static final PolicyFactory TABLES = (new HtmlPolicyBuilder()).allowStandardUrlProtocols()
			.allowElements("table").allowAttributes("border", "cellspacing", "cellpadding").onElements("table")
			.requireRelNofollowOnLinks().toFactory();

	private static final PolicyFactory CLEAR = (new HtmlPolicyBuilder())
			.allowElements(allowedTags)
			.allowElements(new ElementPolicy() {
				@Override
				public String apply(String elementName, List<String> attributes) {
			...
			}, needTransformTags).allowAttributes("src", "width", "height", "href", "target")
			.onElements(linkTags)
			.allowUrlProtocols("https").toFactory();
	...
	public static String sanitizeHtml(String htmlContent) {
		if (htmlContent != null) {
			final PolicyFactory policy = Sanitizers.FORMATTING.and(LINKS).and(Sanitizers.BLOCKS).and(Sanitizers.TABLES)
					.and(IMAGES).and(Sanitizers.STYLES).and(TABLES).and(CLEAR);

			htmlContent = policy.sanitize(htmlContent);
		}
		return htmlContent;
	}
}
