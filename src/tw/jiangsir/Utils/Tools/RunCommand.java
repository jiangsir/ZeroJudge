...
public class RunCommand implements Runnable {
	...
	private void exec(String[] cmd) {
		...
		String info = "";
		info += "開始系統指令 cmd[]=" + cmd[0] + " " + cmd[1] + " " + cmd[2] + "\n";
		logger.info(info);
		...
		try {
			...
			try {
				...
				info += "完成!系統程式。command=" + cmd[2] + "完成!!" + "\n";
			} catch (InterruptedException e) {
				info += "中斷!執行系統程式。command=" + cmd[2] + " 中斷 !!:" + e.getLocalizedMessage() + "\n";
				logger.info(info);
				Log log = new Log();
				log.setTabid(Log.TABID.WATCHING);
				log.setTitle("執行系統程式失敗！(" + cmd[2] + ")");
				log.setMessage("command=" + cmd[2] + " 中斷!!:" + e.getLocalizedMessage());
			...
			this.errStream.add("執行系統指令出錯! " + info);
			logger.severe("執行系統指令出錯! " + info + "\nIOException: RunCommand=" + cmd[2] + ", time="
	...
	/**
	 * 回覆 tomcat6 or tomcat7 or tomcat8 or tomcat9
	 * 
	 * @return
	 */
	public String getTomcatVersion() {
		this.exec(Command.sudo + " " + Command.find + " /etc/ -maxdepth 1 -name tomcat*");
		ArrayList<String> out = this.getOutStream();
		if (out == null || out.size() == 0 || "".equals(out.get(0).toString().trim())) {
			return "";
		} else {
	...
	public String execWhoami() {
		this.exec("python3 " + ApplicationScope.getAppConfig().getBinPath() + File.separator + "whoami.py");
	...
	public String execUptime() {
		this.exec("python3 " + ApplicationScope.getAppConfig().getBinPath() + File.separator + "uptime.py");
	...
	public double getLoadAverage() {
		this.exec("python3 " + ApplicationScope.getAppConfig().getBinPath() + File.separator + "LoadAverage.py");
...
//		this.exec(Command.sudo + " systemctl restart mysql");
		this.exec("python3 " + ApplicationScope.getAppConfig().getBinPath() + File.separator + "restartMysql.py");
	}
...
//		this.exec(Command.sudo + " " + Command.find
//				+ " /etc/init.d/ -name \"tomcat*\" -exec bash -c \"{} restart\" \\;");
//		String[] ss = this.getTomcatVersion().split("/");
//
//		this.exec(Command.sudo + " systemctl restart " + ss[ss.length - 1]);
		this.exec("python3 " + ApplicationScope.getAppConfig().getBinPath() + File.separator + "restartTomcat.py");
	}
	...
	public void execReboot() {
		this.exec("python3 " + ApplicationScope.getAppConfig().getBinPath() + File.separator + "reboot.py");
//
